...
static int mtk_fm_i2s_awb_alsa_stop(struct snd_pcm_substream *substream)
{
	...
	if (!mAwbStarted) {
		pr_warn_once("mtk_fm_i2s_awb_alsa_stop before start; skip hardware disable\n");
		return 0;
	}
	mAwbStarted = false;
...
static int mtk_fm_i2s_awb_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
		...
		pr_warn_once("mtk_fm_i2s_awb_pcm_open non-capture stream; no-op\n");
		return 0;
	}
...
static int mtk_fm_i2s_awb_pcm_close(struct snd_pcm_substream *substream)
{
	if (mAwbClockPrepared) {
		...
		pr_warn_once("mtk_fm_i2s_awb_pcm_close before clock prepare; skip hardware disable\n");
	}
	return 0;
}
