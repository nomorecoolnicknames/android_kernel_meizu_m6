...
readback loop inside the M6 LCM pinctrl diagnostic helper. This patch does
not change any pinctrl state selection, panel reset/bias call, DSI command,
LCM table entry, CMDQ wait, route, timing, or backlight value. It only stops
the diagnostic helper from reading hardcoded GPIO numbers through the MTK GPIO
core, because each such read triggers `mt_gpio_pin_decrypt()` warning and a
full `dump_stack()` on this tree.

Hypothesis: FACT: #96 moved the display off/on cycle past the old
pre-panel wedge: fresh logs contain `M6 LCM pm[suspend-entry]`,
`M6 LCM pm[suspend-exit]`, `M6 LCM pm[resume-entry]`, `M6 LCM resume end`,
and `M6 primary state: SLEPT -> ALIVE`; the old `Wait No Clear Event:
CMDQ_EVENT_DISP_RDMA0_EOF` signature is absent. FACT from the human observer:
when wake was issued, the glass visibly went off/dark, unlike the old
"nothing changed" cycle. FACT: the same capture later rebooted with
`ro.boot.bootreason=wdt_by_pass_pwk`; pstore shows the WDT window after
`M6C19 resume-entry` / `M6C22 resume-before-callback`, with repeated stacks
under `lcm_resume -> lcm_init -> lcm_reset_settting`. FACT: symbol decoding
against #96 `System.map` maps the WDT PC to `disp_irq_handler`
(`ddp_irq.c:403`) and the stack tail through `DSI_dcs_read_lcm_reg_v2`,
`DSI_dcs_read_lcm_reg_v2_wrapper_DSI0`, and `push_table`. FACT: the capture
also contains roughly 90 `primary_display_resume` stack dumps and 87
`lcm_reset_settting` stack dumps generated by `GPIOxx HARDCODE warning!!!`
inside `mt_gpio_pin_decrypt()`, all caused by `m6_lcm_gpio_dump_one()` calling
`mt_get_gpio_mode/dir/out/in/pull_*` on raw hardcoded pins during every
LCM reset/bias step. HYPOTHESIS: before judging the next resume failure, the
diagnostic GPIO readback flood must be removed; otherwise pstore/WDT timing is
contaminated by self-inflicted stack dumping inside the panel resume path.

Evidence:
- Source patch under test: this #97 section and the boot-only artifact below.
- Pre-patch runtime capture:
  `/srv/forge/android/meizu_m6/captures/20260609-1530-m6-rdma-eof-mutex-isolation-offon-711HEBSR277K5/`.
- #96 boot image verified/flashed before the capture:
  ...
  sha256 `f2f3439a4bae87d517beba115804f508bd770a6b52da1cc51d790f85367154cd`.
- Fresh #96 current-boot log:
  `dmesg-after-reappear.txt` lines contain `M6 LCM pm[resume-entry]`,
  `M6 LCM resume end`, `M6 LCM pm[resume-exit]`, and
  `M6 primary state: SLEPT(0xf1) -> ALIVE(0xf0)`.
- Fresh #96 pstore/last_kmsg:
  `last_kmsg-after-reappear.txt` marker tail shows `M6C19 resume-entry`,
  `M6C20 resume-before-power`, `M6C21 resume-after-power`,
  `M6C22 resume-before-callback`, then `WDT_CPU0`.
- Symbol decode, using the exact #96 `vmlinux`/`System.map`: WDT PC
  `ffffffc00057fdb8` = `disp_irq_handler` at `ddp_irq.c:403`;
  stack addresses include `DSI_dcs_read_lcm_reg_v2`,
  `DSI_dcs_read_lcm_reg_v2_wrapper_DSI0`, `push_table`,
  `lcm_init`, `lcm_resume`, `disp_lcm_resume`, and
  `primary_display_resume`.
- #97 boot-only artifact:
  `/srv/forge/android/export/meizu_m6_artifacts/20260609-1548-m6-lcm-gpio-dumpstack-quiesce-bootonly/boot-m6-lcm-gpio-dumpstack-quiesce-20260609.img`.
- #97 boot image sha256:
  `bc952829782432faecbb004d572860356c2dba7c774be8f1772b90f31634ba45`.
- #97 `Image.gz-dtb` sha256:
  `6c3d79cd6ca6151472f16cc15544c3ecc49312ee4b363c40546ff4dc91f461b6`.
- #97 `System.map` sha256:
  `931343cc289f6d1ec1985ec9a01b7797dc79c366432ffe2215c3eaf0e4f636ae`.
- Build/packaging verification: `git diff --check` passed, `make
  Image.gz-dtb` completed, `abootimg -i` reports unchanged 16 MiB boot
  geometry/cmdline, unpacked `zImage` and `initrd.img` compare with packaged
  inputs, `sha256sum -c SHA256SUMS` passed, and marker string search finds
  the new `raw mt_get_gpio reads suppressed` diagnostic text plus the #96
  CMDQ/LCM marker strings.

Files changed:
- `kernel-3.18/drivers/misc/mediatek/video/mt6755/mtkfb.c`: changes
  `m6_lcm_gpio_dump()` from five raw `mt_get_gpio_*` readback calls into a
  bounded "raw reads suppressed" marker. `lcm_pinctl_gpio_output()` and
  `lcm_pinctl_select_state_checked()` still execute the same pinctrl state
  transitions and still log the selected state/result.
...
Why each file changed: `mtkfb.c` owns the local `lcm_pinctl_gpio_output()`
implementation used by the active ILI9881P LCM driver's reset and bias helpers.
The GPIO core itself is not patched because its hardcode warning may be useful
elsewhere; the unsafe caller is the display diagnostic helper that repeats raw
GPIO reads on every LCM reset/bias transition. The state file is the durable
M6 bring-up record required for this diagnostic patch.

Expected next marker: after flashing #97 and running one brightness-pinned
off/wake cycle, dmesg/pstore should no longer contain the repeated
`GPIOxx HARDCODE warning!!!` / `mt_gpio_pin_decrypt` / `dump_stack` flood from
`m6_lcm_gpio_dump_one`. It should contain at most the bounded
`M6 gpio[...] raw mt_get_gpio reads suppressed` lines, plus the normal
`M6 output state=... ret=...` pinctrl result lines. If the device still WDTs
on resume, the remaining frontier is no longer this diagnostic stack flood;
collect the next pstore and inspect the LCM init table / DSI read/IRQ path
around `push_table` and `disp_irq_handler`.

Rollback condition: revert this patch if pinctrl state transitions stop being
visible, panel reset/bias state cannot be correlated with the LCM markers, or
a fresh #97 capture shows a new regression that is clearly caused by removing
the raw GPIO readback diagnostics. Do not revert only because the black panel
remains black; that would only restore the stack-flood contaminant.
...
ART=/srv/forge/android/export/meizu_m6_artifacts/20260609-1548-m6-lcm-gpio-dumpstack-quiesce-bootonly
(cd "$ART" && sha256sum -c SHA256SUMS)
cmp "$ART/Image.gz-dtb" "$ART/verify-unpack.tmp/zImage"
cmp "$ART/initrd.img" "$ART/verify-unpack.tmp/initrd.img"
abootimg -i "$ART/boot-m6-lcm-gpio-dumpstack-quiesce-20260609.img"
grep -E 'raw mt_get_gpio reads suppressed|M6 CMDQ video eof isolation|M6 LCM pm' "$ART/marker-strings.txt"
```
...
`m6_dsi_c2v_switch` CPU-direct path, skip exactly the next proven reset
boundary: the C2V `MUTEX0_EN` release write at `0xF4020020` after
`DSI_Start`. The normal path keeps its existing `MUTEX0_EN` write. This patch
does not change boot-time display sequencing, panel init tables, LCM reset,
DDP route construction, OVL/RDMA programming, PQ/HWC, clocks, or normal CMDQ
scanout. It only lets the debugfs C2V probe continue past the second
DDP-mutex write so the next lower DSI/panel state can be captured.

Hypothesis: FACT: #85 boot image
`47f2ddb91b6c994403e4b0b3d4a3d7e41d737a488404d38bf665e3efa0f9ec65` was
...
`/srv/forge/android/meizu_m6/captures/20260609-0958-m6-dsi-c2v-skip-mutex-video-isolation-root-c2v`
booted as `Linux localhost 3.18.140 #85`, reached `sys.boot_completed=1`, and
matched the #85 boot hash before the root C2V run. FACT: after the root
`m6_dsi_c2v_switch:0x03:1500` command, the device rebooted with WDT status
`2`, fiq step `32`, exception type `2`, and still matched the #85 boot hash
after reboot. FACT: fresh `proc-last_kmsg.txt` and `pstore.txt` now show
`M6K01 c2v-skip-mutex-video`, `c2v-after-pkt0`, `c2v-after-pkt1`,
`c2v-after-vmstart-reg`, and `c2v-after-dsi-start`, then no
`c2v-after-mutex-release`. FACT: `0xF4020020` is
`DISP_REG_CONFIG_MUTEX0_EN` in the current source. INFERENCE: #85 proved the
previous `MUTEX0_SOF` write was a real boundary but not the only DDP-mutex
danger; the probe now reaches DSI packet programming, VM start, and
`DSI_Start`. HYPOTHESIS: skipping only the debugfs CPU-direct `MUTEX0_EN`
release write will show whether the second WDT is caused by releasing
`MUTEX0` in this C2V state, or whether the command can complete and reveal a
later DSI host / MIPITX / panel electrical failure.
...
- #86 boot sha256:
  `1071cafdaf3e0c5c43266946f18ca0f8b4496f95e5f5ed3a6110034084187857`.
- #86 `Image.gz-dtb` sha256:
  `a175e47003bebbf6fd89081fd5ac38f2f400540ad7c7d986c8b8880c3ccc2c80`.
- #86 `System.map` sha256:
  `97bc288f554ae759f767714c856291f77bf6e1bd021b0b3e6d1930b0b6522bf1`.
- #86 packaging verification: `abootimg --create`, `abootimg -i`,
  `sha256sum -c`, unpacked `zImage` compare, unpacked `initrd.img` compare,
  and marker-string check passed. `gzip` reported only expected trailing DTB
  garbage after a successful decompression.
- Runtime capture:
  `/srv/forge/android/meizu_m6/captures/20260609-0958-m6-dsi-c2v-skip-mutex-video-isolation-root-c2v`.
- Post-reboot evidence:
  `/srv/forge/android/meizu_m6/captures/20260609-0958-m6-dsi-c2v-skip-mutex-video-isolation-root-c2v/post-valid-c2v-reboot-evidence`.
- #85 last markers:
  `M6K01 c2v-skip-mutex-video E=1 O=5f280 F=41 S=10001 M=80003 I=780`,
  `M6K01 c2v-after-pkt0`, `M6K01 c2v-after-pkt1`,
  `M6K01 c2v-after-vmstart-reg`, and
  `M6K01 c2v-after-dsi-start S=10001 M=80003 I=80000780`.

Files changed:
- `kernel-3.18/drivers/misc/mediatek/video/mt6755/ddp_dsi.c`: keeps the
  existing `MUTEX0_EN` release write for non-null `cmdq_handle`, skips that
  single write only in the M6 debugfs CPU-direct C2V probe, and logs
  `MUTEX0_{EN,MOD,SOF}` plus DSI start/mode/int state.
- `BRINGUP_STATE.md`: records the #85 result, the new reset boundary, and the
  expected #86 marker.

Why each file changed: `ddp_dsi.c` owns the now-proven post-`DSI_Start`
reset instruction group. The state file keeps future work from treating
`MUTEX0_SOF`, DSI packets, or `DSI_Start` as still untested.

Expected next marker: after flashing #86 and rerunning
`m6_dsi_c2v_switch:0x03:1500` as root, logs should include
`M6Kxx c2v-skip-mutex-release`, `switch-dsi-after-c2v-start`,
`M6 DSI switch_mode C2V: cpu-direct path skip cmdq flush/reset/wait`,
`switch-dsi-exit`, `c2v-switch-after-dsi`, and `c2v-switch-hold-end`. If the
device still resets before `c2v-skip-mutex-release`, the `MUTEX0_EN` read/base
access itself is suspect. If it resets after that marker, the next missing
marker names the lower boundary after the second DDP mutex write is removed.

Rollback condition: revert this isolation if verified #86 regresses normal
boot before the debugfs command, still resets before the new skip marker, or
skipping both debugfs mutex writes produces no new lower-layer evidence. Do
not present this as a display fix unless later evidence proves stable normal
scanout and a visible panel.
...
$A root
$A wait-for-device
$A shell 'id; sha256sum /dev/block/platform/mtk-msdc.0/by-name/boot'
$A shell 'dmesg -C'
