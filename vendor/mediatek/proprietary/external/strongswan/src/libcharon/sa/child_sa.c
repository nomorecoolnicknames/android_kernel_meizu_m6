	...
	linked_list_t *my_ts, linked_list_t *other_ts)
{
	...
	if (inbound)
	{
		...
		{
			if (dst_ts->get_type(dst_ts) == TS_IPV6_ADDR_RANGE)
			{
				DBG1(DBG_IKE, "origin inbound policy,dst : %R", dst_ts);
				if (this->config->use_force_tsi_64(this->config) && dst_ts) {
	...
	else
	{
		...
		{
            if (src_ts->get_type(src_ts) == TS_IPV6_ADDR_RANGE)
            {
			    DBG1(DBG_IKE, "origin outbound policy,src : %R", src_ts);
    			if (this->config->use_force_tsi_64(this->config) && src_ts) {
