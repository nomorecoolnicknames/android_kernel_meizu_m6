...
/**
 * Call the charon controller to notify the data path status change
 */
static void charon_data_path_status_change(private_stroke_control_t *this,
							 stroke_msg_t *msg, FILE *out, bool data_path_ready)
{
	if (data_path_ready)
	{
		hydra->kernel_interface->notify_data_path(hydra->kernel_interface, msg->data_path_status_notify.if_name);
		DBG1(DBG_CFG, "enable_ike_data_path interface %s", msg->data_path_status_notify.if_name);
	}
	else
	{
		DBG1(DBG_CFG, "disable_ike_data_path");
	}
	...
	private_stroke_control_t *this, stroke_msg_t *msg, FILE *out)
{
		...
		ike_list->get_count(ike_list) == 0)
	{
		...
		notify_wod(N_DETACH, name + 4, NULL);
	}
	ike_list->destroy(ike_list);
	child_list->destroy(child_list);
}

METHOD(stroke_control_t, data_path_status_notify, void,
	private_stroke_control_t *this, stroke_msg_t *msg, FILE *out)
{

	ike_sa_t *ike_sa;
	enumerator_t *enumerator;
	linked_list_t *ike_list;
	uintptr_t id;
	bool data_path_ready = FALSE;

	if(!strcmp(msg->data_path_status_notify.status, "up"))
	{
		data_path_ready =TRUE;
		DBG1(DBG_CFG, "data path is ready,if_name %s", msg->data_path_status_notify.if_name);
	}
	else if(!strcmp(msg->data_path_status_notify.status, "down"))
	{
		data_path_ready =FALSE;
		DBG1(DBG_CFG, "data path is not ready,if_name %s", msg->data_path_status_notify.if_name);
	}
	else
	{
		DBG1(DBG_CFG, "data path status is not valid");
	}
	charon_data_path_status_change(this, msg, out, data_path_ready);
}
...
stroke_control_t *stroke_control_create()
{
	...
	INIT(this,
		.public = {
			...
			.unroute = _unroute,
			.destroy = _destroy,
			.data_path_status_notify = _data_path_status_notify,
		},
