...
/// FUNCTION_NAME:add_mem_info
//This infomation from command line
#define BUF_MAX_LENGTH 32
static void add_mem_info(void)
{
    char ddr_info_all[BUF_MAX_LENGTH]={0};
    char emmc_info_all[BUF_MAX_LENGTH]={0};
    char *p_str1,*p_str2;
    char *sub_ddr_str = "androidboot.hardware.ddr=";
    char *sub_emmc_str = "androidboot.hardware.emmc=";
    int length = 0;

    p_str1 = strstr(boot_command_line,sub_ddr_str);
    if(p_str1 == NULL)
        return;
    p_str1 += strlen(sub_ddr_str);
    p_str2 = strstr(p_str1," ");
    if(p_str2 == NULL)
        return;
    length = p_str2 - p_str1;
    memcpy(ddr_info_all,p_str1,length);
    app_info_set("ddr_vendor",ddr_info_all);

    p_str1 = strstr(boot_command_line,sub_emmc_str);
    if(p_str1 == NULL)
        return;
    p_str1 += strlen(sub_emmc_str);
    p_str2 = strstr(p_str1," ");
    if(p_str2 == NULL)
        return;
    length = p_str2 - p_str1;
    if( length > BUF_MAX_LENGTH)
        length = BUF_MAX_LENGTH;
    memcpy(emmc_info_all,p_str1,length);
    app_info_set("emmc_vendor",emmc_info_all);
}
