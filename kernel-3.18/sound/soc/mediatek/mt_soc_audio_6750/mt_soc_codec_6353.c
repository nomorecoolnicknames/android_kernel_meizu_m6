...
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	...
	if (enable) {
...
#else
#if defined(CONFIG_EXTPA_AW87318)
...
#endif
		//modify by caozhg to enable APA_EN
		//AudDrv_GPIO_EXTAMP_Select(true, 3);
#if defined(CONFIG_AUDIO_DEBUG)
        printk("Ext_Speaker_Amp_Change ON-\n");
#else
		...
		udelay(500);
#endif
		//modify by caozhg to disable APA_EN
		//AudDrv_GPIO_EXTAMP_Select(false, 3);
#if defined(CONFIG_AUDIO_DEBUG)
        printk("Ext_Speaker_Amp_Change OFF-\n");
#else
	...
	if (enable) {
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {	/* main and headset mic */
			...
			/* Audio R preamplifier DCCEN */
                    ...
                    /* "ADC2", headset mic */
                    SetDCcoupleNP(AUDIO_MIC_BIAS1, mAudio_Analog_Mic2_mode);
                    /* micbias1 DCCopuleNP */
                    if (ECMmode == 1) {	/* differential */
                        Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0770, 0xfff0);
                    /* Enable MICBIAS1, SwithP/N MISBIAS1 = 2P5V */
                    ...
                    Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0004, 0xffff);
                    /* Audio R preamplifier DCC precharge */
                    Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0085, 0xffff);
                    /* Audio R preamplifier input sel : AIN2. Enable audio R PGA */
                    /* Ana_Set_Reg(AUDENC_ANA_CON15, 0x0020, 0x00f0); //Audio R PGA 12 dB gain */
                    Ana_Set_Reg(MT6353_AUDENC_ANA_CON10, 0x0033, 0x00ff);
                    /* Audio R PGA 18 dB gain MT6328 */
                    Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0087, 0xffff);
                    /* Audio R preamplifier DCCEN */
            }
		}
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {
            ...
            if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
                ...
                /* headset mic */
                Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0587, 0xffff);
                /* Audio R ADC input sel : R PGA. Enable audio R ADC */
                udelay(100);
                Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0583, 0xffff);
                /* Audio R preamplifier DCC precharge off */
            }
		}
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {
			...
			/* disable MICBIAS0 and MICBIAS1, restore to micbias set by accdet */
                ...
                /* headset mic */
                Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0083, 0xffff);
                /* Audio R ADC input sel : off, disable audio R ADC */
                Ana_Set_Reg(MT6353_AUDENC_ANA_CON3, 0x0000, 0xffff);
                /* PGA stb enhance off */
                Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0081, 0xffff);
                /* Audio R preamplifier DCCEN disable */
                Ana_Set_Reg(MT6353_AUDENC_ANA_CON10, 0x0000, 0x00f0);
                /* Audio R PGA 0 dB gain */
                Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0000, 0xffff);
...
#if defined(CONFIG_EXTPA_AW87318)
    int rt;
    rt = gpio_request(LCT_AW87318_PIN_EN,NULL);
    if(rt){
#ifdef CONFIG_HUAWEI_DSM
        audio_dsm_report_info(DSM_AUDIO_MODEM_CRASH_CODEC_CALLBACK,
            "audio ext amp gpio request failed");
#endif
#if defined(CONFIG_AUDIO_DEBUG)
        printk("LCT_AW87318_PIN_EN request failed");
#else
        pr_err("LCT_AW87318_PIN_EN request failed");
#endif
    }
    rt = gpio_direction_output(LCT_AW87318_PIN_EN,GPIO_DIR_OUT);
    if(rt){
#ifdef CONFIG_HUAWEI_DSM
        audio_dsm_report_info(DSM_AUDIO_MODEM_CRASH_CODEC_CALLBACK,
            "audio ext amp gpio direction output failed");
#endif
        pr_err("LCT_AW87318_PIN_EN gpio direction output failed");
    }
#endif
...
#ifdef CONFIG_HUAWEI_DSM
        audio_dsm_report_num(DSM_AUDIO_CARD_LOAD_FAIL_ERROR_NO,
                DSM_AUDIO_MESG_CODEC_PROBE_FAIL);
#endif
		return -ENOMEM;
