				...
				void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (DMA_Read_Ptr >=  Awb_Block->u4BufferSize) {
        PRINTK_AUD_AWB("%s 1, DMA_Read_Ptr out of bound.\n", __func__);
	...
	if (DMA_Read_Ptr + read_size <= Awb_Block->u4BufferSize) {
        ...
        PRINTK_AUD_AWB("%s 1, read_size:%zu, Remained:0x%x, Read_Ptr:%zu, ReadIdx:0x%x \r\n",
		...
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
            PRINTK_AUD_AWB("%s 2, read_size1:0x%x, DataRemained:0x%x, Ptr:%zu, ReadIdx:0x%x \r\n",
		...
		if (DMA_Read_Ptr >= Awb_Block->u4BufferSize) {
            PRINTK_AUD_AWB("%s 2, DMA_Read_Ptr out of bound.\n", __func__);
			DMA_Read_Ptr %= Awb_Block->u4BufferSize;
		}
		spin_unlock(&auddrv_Dl1AWBInCtl_lock);
