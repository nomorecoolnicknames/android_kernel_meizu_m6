					 ...
					 char *page)
{
	...
	int i;
	int count = 0;
	char cur_attr_buff[128] = {0};
    int cur_attr_len = 0;
	mutex_lock(&pkgl_data_all->hashtable_lock);
	hash_for_each_safe(pkgl_data_all->package_to_appid, i, h_t, hash_cur, hlist)
	{
	memset(cur_attr_buff, 0, sizeof(cur_attr_buff));
	cur_attr_len = snprintf(cur_attr_buff, sizeof(cur_attr_buff), "%s %d\n", (char *)hash_cur->key, hash_cur->value);
	if (count + cur_attr_len < PAGE_SIZE)
	{
