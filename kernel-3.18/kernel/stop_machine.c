...
static int __init cpu_stop_init(void)
{
	...
	forge_m681_mark(0xF6);
	BUG_ON(smpboot_register_percpu_thread(&cpu_stop_threads));
	forge_m681_mark(0xF7);
...
/*
 * stop_machine_park - park the stopper thread for @cpu during hotplug offline.
 * Called from cpu.c take_cpu_down() with the stopper still live.
 * Sets enabled=false so no new works are queued, then parks the kthread.
 * The stopper's smpboot park callback (cpu_stop_park) will drain any
 * already-queued works when the thread actually parks.
 */
void stop_machine_park(int cpu)
{
	 ...
	 */
	stopper->enabled = false;
	kthread_park(per_cpu(cpu_stopper_task, cpu));
}
