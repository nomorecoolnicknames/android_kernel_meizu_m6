...
unsigned int T_R_CHARGER_1 = R_CHARGER_1;
#define GPIO54 54
...
unsigned int battery_type = UNSTANDAND_BATTERY;
...
void fgauge_get_profile_id(void)
{
	...
	int id = 0;
	int ret = 0;
	kal_bool unstandard_battery = KAL_FALSE;
	...
	for (id = 0; id < TOTAL_BATTERY_NUMBER; id++) {
		if (id_volt > g_battery_id_voltage[id] && g_battery_id_voltage[id] != -1) {
			g_fg_battery_id = id;
			unstandard_battery = KAL_FALSE;
			break;
		} else {
			g_fg_battery_id = 0;
			unstandard_battery = KAL_TRUE;
		}
	}
	...
	{
		case 0x0:
			if(unstandard_battery == KAL_TRUE){
				app_info_set("battery", "UNSTANDAND");
				battery_type = UNSTANDAND_BATTERY;
			}
			else{
				app_info_set("battery", "DESAY");
				battery_type = DESAY;
			}
			break;
		case 0x1:
			app_info_set("battery", "SUNWODA");
			battery_type = SUNWODA;
			break;
		default:
			break;
	}
...
int __batt_meter_init_cust_data_from_cust_header(struct platform_device *dev)
{
	...
	batt_meter_cust_data.r_charger_1 = T_R_CHARGER_1;
	g_R_CHARGER_1 = T_R_CHARGER_1;
...
//modify by caozhg to support multi battery
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
	...
	batt_meter_cust_data.trk_point_thr = CUST_TRACKING_GAP;
	batt_meter_cust_data.trk_point_en = CUST_TRACKINGEN;
...
int ChrThermistorConverTemp(int Res)
{
	int i = 0;
	int RES1 = 0, RES2 = 0;
	int TChr_Value = 0, TMP1 = 0, TMP2 = 0;

	if (Res >= Chr_Temperature_Table[0].TemperatureR) {
		TChr_Value = -25;
	} else if (Res <= Chr_Temperature_Table[RCHR_TEMP_TABLE_LEN].TemperatureR) {
		TChr_Value = 125;
	} else {
		RES1 = Chr_Temperature_Table[0].TemperatureR;
		TMP1 = Chr_Temperature_Table[0].ChrTemp;

		for (i = 0; i <= RCHR_TEMP_TABLE_LEN; i++) {
			if (Res >= Chr_Temperature_Table[i].TemperatureR) {
				RES2 = Chr_Temperature_Table[i].TemperatureR;
				TMP2 = Chr_Temperature_Table[i].ChrTemp;
				break;
			}
			RES1 = Chr_Temperature_Table[i].TemperatureR;
			TMP1 = Chr_Temperature_Table[i].ChrTemp;

		}

		TChr_Value = (((Res - RES2) * TMP1) + ((RES1 - Res) * TMP2)) / (RES1 - RES2);
	}

	return TChr_Value;
}

signed int battery_meter_get_chr_temperature(void)
{
	int chr_temp_volt = 0;
	int ret = 0;
	int Tchr = 0, Tres = 0;

	ret = IMM_GetOneChannelValue_Cali(ADC_ID_CHR_TEMP, &chr_temp_volt);
	if (ret != 0)
		bm_err ("[battery_meter_get_chr_temperature]chr_temp_volt read fail\n");
	else
		bm_err ("[battery_meter_get_chr_temperature]chr_temp_volt = %d\n", chr_temp_volt);

	chr_temp_volt = chr_temp_volt / 1000;
	Tres = ((RCHR_PULL_UP_R * chr_temp_volt) / (RCHR_PULL_UP_VOLT - chr_temp_volt)) * 1000;

	Tchr = ChrThermistorConverTemp (Tres);
	if ((Tchr >= 125) || (Tchr <= -25)) {
		bm_err("[battery_meter_get_chr_temperature] Tchr = %d.\n", Tchr);
	}
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	rc = mt_get_gpio_in(GPIO_BOARD_ID);
	battery_log(BAT_LOG_FULL,"GPIO_BOARD_ID value is %d\n",rc);
	if(rc == 0){
		T_R_CHARGER_1 = 300;
	}
	/* LOG System Set */
...
void bmd_ctrl_cmd_from_user(void *nl_data, struct fgd_nl_msg_t *ret_msg)
{
...
#endif
			if (BMT_status.UI_SOC2 == 0 && (get_boot_mode() != LOW_POWER_OFF_CHARGING_BOOT && get_boot_mode() != KERNEL_POWER_OFF_CHARGING_BOOT))
			{
	...
	case FG_DAEMON_CMD_IS_HW_OCV_UPDATE:
		...
		{
			signed int cali_car_tune = 0;
			memcpy(&cali_car_tune, &msg->fgd_data[0], sizeof(cali_car_tune));
			bm_err("[fg_res] cali_car_tune = %d(%d), default = %d, ioctl:%d Use[cali_car_tune]\n",
				cali_car_tune, (cali_car_tune + 5) / 10, batt_meter_cust_data.car_tune_value,
				ioctl_car_tune);
			if (ioctl_car_tune == false)
				batt_meter_cust_data.car_tune_value = (cali_car_tune + 5) / 10;
			is_car_tune_rdy = true;
		}
		break;
