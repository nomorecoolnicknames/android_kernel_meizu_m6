...
#include <mt-plat/mt_gpio.h>
#define GPIO54 54
#define GPIO_BOARD_ID (GPIO54|0x80000000)
int board_gpio54 = 0;
#ifdef CONFIG_ATO_TEST
extern int otg_adc_value[5];
#endif
...
#endif
#endif
#define V_CHARGER_MAX_7 7000
#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_20_SUPPORT)
#ifndef PUMP_EXPRESS_SERIES
#define PUMP_EXPRESS_SERIES
#endif
#endif
extern unsigned int battery_type;
...
extern kal_bool g_read_write;
#define MAX_BATTERY_VOLTAGE 4520
#define MAX_CHARGE_VOLTAGE 6200
#define MAX_CURRENT 20500
...
#define Get_META_BAT_CURRENT _IOW('k', 15, int)
/* add for meta tool----------------------------------------- */
bool ioctl_car_tune;
bool is_car_tune_rdy;
...
int charger_detect_register_client(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&charger_notifier_list, nb);
}
EXPORT_SYMBOL(charger_detect_register_client);

int charger_detect_unregister_client(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&charger_notifier_list, nb);
}
EXPORT_SYMBOL(charger_detect_unregister_client);
int charger_detect_notifier_call_chain(unsigned long val, void *v)
{
	return blocking_notifier_call_chain(&charger_notifier_list, val, v);
}
#define CHARGER_EXIST		1
#define CHARGER_NOT_EXIST	0
/* ////////////////////////////////////////////////////////////////////////////// */
/* FOR ADB CMD */
/* ////////////////////////////////////////////////////////////////////////////// */
/* Dual battery */
...
/* ///////////////////////////////////////////////////////////////////////////////////////// */
int lct_get_manul_charger_onoff(void);
void lct_set_manul_charger_onoff(int flag);
static int lct_manul_charger_onoff=0;
kal_bool upmu_is_chr_det(void)
{
#if !defined(CONFIG_POWER_EXT)
	unsigned int tmp32;
#endif
	if(lct_manul_charger_onoff ==1)
	{
		return KAL_FALSE;
	}
	if (!g_bat_init_flag) {
...
/*POWER_SUPPLY_PROP_CURRENT_NOW same as POWER_SUPPLY_PROP_BatteryAverageCurrent,for huawei_charger*/
	...
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		...
		break;
	case POWER_SUPPLY_PROP_Charge_Current_Max:
		val->intval = MAX_CHARGE_CURRENT;
		break;
...
static ssize_t show_charging_enabled(struct device *dev, struct device_attribute *attr, char *buf)
{
    battery_log(BAT_LOG_CRTI, "[BATTERY] plugusb : %d\n", g_plugusb);
    return sprintf(buf, "%u\n", g_plugusb);
}

static ssize_t store_charging_enabled(struct device *dev, struct device_attribute *attr,
                      const char *buf, size_t size)
{
    char *pvalue = NULL;
    unsigned int plugusb = 0;
    battery_log(BAT_LOG_CRTI,"[BATTERY] adaptor_online\n");

    if(buf!=NULL && size!=0)
    {
        battery_log(BAT_LOG_CRTI,"[BATTERY] buf is %s.\n",buf);
        plugusb=simple_strtol(buf,&pvalue,10);
        if((plugusb == 0)||(plugusb == 1))
            g_plugusb = plugusb;
        else
            battery_log(BAT_LOG_CRTI,"eg:echo[0/1]>charging_enabled.\n");
		wake_up_bat();
        battery_log(BAT_LOG_CRTI,"[BATTERY] store code :%d\n",g_plugusb);
    }
    return size;
}
static DEVICE_ATTR(charging_enabled, 0664, show_charging_enabled, store_charging_enabled);


static ssize_t show_battery_temp_limitation(struct device *dev, struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_CRTI, "[Battery] show_battery_temp_limitation : %x\n", battery_temp_limitation);
	return sprintf(buf, "%u\n", battery_temp_limitation);
}

static ssize_t store_battery_temp_limitation(struct device *dev, struct device_attribute *attr, const char *buf,
				 size_t size)
{
	unsigned int reg_tempLimit = 0;
	int ret;

	battery_log(BAT_LOG_CRTI, "[Battery] store_battery_temp_limitation\n");
	if (buf != NULL && size != 0) {
		battery_log(BAT_LOG_CRTI, "[Battery] buf is %s and size is %Zu\n", buf, size);
		ret = kstrtouint(buf, 16, &reg_tempLimit);
		battery_temp_limitation = reg_tempLimit;
		battery_log(BAT_LOG_CRTI, "[Battery] store battery_temp_limitation : %x\n", battery_temp_limitation);
	}
	return size;
}

static DEVICE_ATTR(BatteryTL, 0664, show_battery_temp_limitation, store_battery_temp_limitation);
...
{
	battery_log(BAT_LOG_CRTI, "[BATTERY] charging current : %d\n", g_set_charging_current);
	return sprintf(buf, "%d\n", g_set_charging_current);
}
...
{
	if (buf != NULL && size != 0) {
		if (sscanf(buf, "%d", &g_set_charging_current) == 1) {
			g_set_charging_current*=100;
		}
	}
	return size;
}
...
{
	if (buf != NULL && size != 0) {
		battery_log(BAT_LOG_CRTI,"lct_manul_charger_onoff buf is %s\n", buf);
		sscanf(buf, "%d", &lct_manul_charger_onoff);
		battery_log(BAT_LOG_CRTI,"lct_manul_charger_onoff=%d\n", lct_manul_charger_onoff);
	}
	return size;
}

static DEVICE_ATTR(manul_charger_onoff, 0664, show_manul_charger_onoff, store_manul_charger_onoff);
static int ontim_charger_hz_onoff = 0;
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	bat_data->BAT_voltage_now = BMT_status.bat_vol_now;
	bat_data->TemperatureChr = BMT_status.temperature_chr;
...
void mt_battery_GetBatteryData(void)
{
	...
	battery_log(BAT_LOG_CRTI, "running test bat_temp = (%d)\n", temperature);
	if (temperature < (-15))
		temperature = -15;
	...
	BMT_status.temperature_chr = battery_meter_get_chr_temperature();
...
static PMU_STATUS mt_battery_CheckBatteryTemp(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[BATTERY] support JEITA, temperature=%d\n",
		    BMT_status.temperature);
	#ifndef CONFIG_LCT_RUNNING_DISABLE_CURRENT_LIMIT
...
static void mt_battery_CheckBatteryStatus(void)
{
	...
	battery_log(BAT_LOG_FULL, "[mt_battery_CheckBatteryStatus] cmd_discharging=(%d)\n",
		    cmd_discharging);
	if (cmd_discharging == 1 || g_plugusb == 1 || adjust_power == 1 || ontim_charger_hz_onoff == 1) {
		battery_log(BAT_LOG_CRTI,
		...
		return;
	} else if (cmd_discharging == 0 || g_plugusb == 0|| adjust_power == 0 || ontim_charger_hz_onoff == 0) {
			battery_log(BAT_LOG_CRTI,
				"%s line %d cmd_discharging=(%d) ontim_charger_hz_onoff=(%d)\n", __func__, __LINE__,
				cmd_discharging, ontim_charger_hz_onoff);
		if(first_enter == 0)
...
#else
	#ifndef CONFIG_LCT_RUNNING_DISABLE_CURRENT_LIMIT
	if(BMT_status.temperature <= TEMP_MIN_THRESHOLD){
		g_BatteryNotifyCode = 0x0400;
		battery_log(BAT_LOG_FULL, "[BATTERY] bat_temp(%d) Below %d degrees\n",BMT_status.temperature, TEMP_MIN_THRESHOLD);
	}else if ((BMT_status.temperature < TEMP_POS_0_THRESHOLD)&&(BMT_status.charger_exist == KAL_TRUE)) {
		g_BatteryNotifyCode = 0x0040;
		battery_log(BAT_LOG_FULL, "[BATTERY] bat_temp(%d) charging Below %d degrees\n", BMT_status.temperature, TEMP_POS_0_THRESHOLD);
	}
	else if ((BMT_status.temperature < TEMP_MAX_THRESHOLD)&&(BMT_status.temperature >TEMP_POS_45_THRESHOLD)) {
		g_BatteryNotifyCode = 0x0080;
		battery_log(BAT_LOG_FULL, "[BATTERY] bat_temp(%d) charging between %d and %d degrees\n",BMT_status.temperature, TEMP_POS_45_THRESHOLD, TEMP_POS_60_THRESHOLD);
	}else if((BMT_status.temperature >= TEMP_MAX_THRESHOLD)&&(BMT_status.charger_exist == KAL_TRUE)&&(battery_temp_limitation == 0)){
		g_BatteryNotifyCode = 0x0100;
		battery_log(BAT_LOG_FULL, "[BATTERY] bat_temp(%d) charging above %d degrees\n",BMT_status.temperature, TEMP_MAX_THRESHOLD);
	}else if((BMT_status.temperature >= TEMP_MAX_THRESHOLD)&&(BMT_status.charger_exist == KAL_FALSE)&&(battery_temp_limitation == 0)){
		g_BatteryNotifyCode = 0x0200;
		battery_log(BAT_LOG_FULL, "[BATTERY] bat_temp(%d) discharging above %d degrees\n",BMT_status.temperature, TEMP_MAX_THRESHOLD);
	}else if((BMT_status.temperature >= TEMP_MAX_THRESHOLD_70)&&(BMT_status.charger_exist == KAL_TRUE)&&(battery_temp_limitation == 1)){
		g_BatteryNotifyCode = 0x0100;
		battery_log(BAT_LOG_FULL, "[BATTERY] bat_temp(%d) charging above %d degrees\n",BMT_status.temperature, TEMP_MAX_THRESHOLD);
	}else if((BMT_status.temperature >= TEMP_MAX_THRESHOLD_70)&&(BMT_status.charger_exist == KAL_FALSE)&&(battery_temp_limitation == 1)){
		g_BatteryNotifyCode = 0x0200;
		battery_log(BAT_LOG_FULL, "[BATTERY] bat_temp(%d) discharging above %d degrees\n",BMT_status.temperature, TEMP_MAX_THRESHOLD);
	}
	else{
		g_BatteryNotifyCode = 0x0000;
		battery_log(BAT_LOG_FULL, "[BATTERY] bat_temp(%d)\n",BMT_status.temperature);
	}
	#endif
#endif
}
...
{
/*modify by caozhg to support sy6923d*/
#if defined(CONFIG_MTK_BQ25896_SUPPORT) || defined(CONFIG_MTK_SY6923D_SUPPORT)
...
extern int mtk_idpin_irqnum;
#define GPIO_BOARD_ID_OTG (GPIO51 | 0x80000000)
void do_chrdet_int_task(void)
{
	...
	int id_volt = 0;
        int rc = 0;
        mt_set_gpio_mode(GPIO_BOARD_ID_OTG, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO_BOARD_ID_OTG, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_BOARD_ID_OTG, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_BOARD_ID_OTG, GPIO_PULL_DOWN);
        rc = mt_get_gpio_in(GPIO_BOARD_ID_OTG);
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
			...
			BMT_status.charger_exist = KAL_TRUE;
			charger_detect_notifier_call_chain(CHARGER_EXIST, NULL);
			wake_lock(&battery_suspend_lock);
			...
			ret = IMM_GetOneChannelValue_Cali(14, &id_volt);
			if (ret != 0)
					printk("[otg_adc]id_volt read fail\n");
				else
					printk("[otg_adc]id_volt = %d\n", id_volt);
		        if (((id_volt <150*1000) || (id_volt >1250*1000))&&(rc==1))
					enable_irq(mtk_idpin_irqnum);
			battery_log(BAT_LOG_CRTI, "[do_chrdet_int_task] charger NOT exist!\n");
			BMT_status.charger_exist = KAL_FALSE;
			charger_detect_notifier_call_chain(CHARGER_NOT_EXIST, NULL);
...
static int dms_chg_monitor_report_err(int errno)
{
	...
	switch (errno)
	{
        ...
        case ERROR_CHARGE_TEMP_FAULT:
            ...
            break;
        case ERROR_CHARGE_I2C_RW:
            dsm_client_record(bq_power_dclient,"Charger i2c read and write fail\n");
            break;
		case DSM_BATTERY_DETECT_ERROR_NO:
			dsm_client_record(bq_power_dclient,"Unstandard battery\n");
        default:
            break;
    }
	size = dsm_client_record(bq_power_dclient,
	"VBUS:%d, VBAT:%d, capacity:%d, present:%d,,batt_temp:%d\n",BMT_status.charger_vol,
	BMT_status.bat_vol, BMT_status.UI_SOC2, BMT_status.bat_exist, BMT_status.temperature);
...
int bat_update_thread_dsm(void)
{
    ...
    if(BMT_status.bat_vol < MIN_BATTERY_VOLTAGE)
    {
        dms_chg_monitor_report_err(ERROR_BATT_VOLT_LOW);
    }
    if(BMT_status.bat_vol > MAX_BATTERY_VOLTAGE)
    {
        dms_chg_monitor_report_err(ERROR_BATT_VOLT_HIGH);
	}
    if((BMT_status.bat_vol < MAX_BATTERY_VOLTAGE) && (abs(BMT_status.IBattery / 10) == 0) &&(BMT_status.charger_type != CHARGER_UNKNOWN))
    {
        dms_chg_monitor_report_err(ERROR_BATT_TERMINATE_TOO_EARLY);
	}
    if((BMT_status.bat_vol > MAX_BATTERY_VOLTAGE) && (BMT_status.charger_type != CHARGER_UNKNOWN))
    {
        dms_chg_monitor_report_err(ERROR_BATT_NOT_TERMINATE);
	}
	if((BMT_status.charger_type != CHARGER_UNKNOWN) && (!abs(BMT_status.IBattery / 10))&&(BMT_status.charger_vol>MAX_CHARGE_VOLTAGE))
	{
		dms_chg_monitor_report_err(ERROR_CHARGE_CURR_ZERO);
	}
    if ((BMT_status.charger_type != CHARGER_UNKNOWN) && (BMT_status.charger_vol > MAX_CHARGE_VOLTAGE))
    {
       dms_chg_monitor_report_err(ERROR_CHARGE_VBAT_OVP);
	}
	if((BMT_status.charger_type != CHARGER_UNKNOWN) &&((BMT_status.temperature < MIN_TEMPERATURE)||(BMT_status.temperature > MAX_TEMPERATURE)))
    {
        dms_chg_monitor_report_err(ERROR_CHARGE_TEMP_FAULT);
    }
    if((BMT_status.charger_type != CHARGER_UNKNOWN) && (abs(BMT_status.IBattery / 10) > MAX_CURRENT))
    {
        dms_chg_monitor_report_err(ERROR_CHARGE_CURR_OVERHIGH);
    }
	if(g_read_write == KAL_FALSE)
	{
		dms_chg_monitor_report_err(ERROR_CHARGE_I2C_RW);
	}
	if(!battery_type)
	{
		dms_chg_monitor_report_err(DSM_BATTERY_DETECT_ERROR_NO);
	}
   return 0;
}
#endif
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case Set_META_BAT_CAR_TUNE_VALUE:
		...
		batt_meter_cust_data.car_tune_value = temp_car_tune;
		ioctl_car_tune = true;
	...
	case Get_META_BAT_CURRENT:
		...
		i = 0;
		while(is_car_tune_rdy == false && i < 20&&ioctl_car_tune==false) {
			msleep(1000);
			i++;
		}

		for (i = 0; i < 60; i++)
			iavg += battery_meter_get_battery_current();
		adc_out_data[0] = iavg / 6000;
		ret = copy_to_user(user_data_addr, adc_out_data, 8);
		pr_err("Get_META_BAT_CURRENT %d %d \n",
			adc_out_data[0],
			is_car_tune_rdy);
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_charging_enabled);
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_BatteryTL);
	}

	/* battery_meter_initial();      //move to mt_battery_GetBatteryData() to decrease booting time */

	mt_set_gpio_mode(GPIO_BOARD_ID, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO_BOARD_ID, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_BOARD_ID, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_BOARD_ID, GPIO_PULL_DOWN);
	mdelay(20);

	board_gpio54 = mt_get_gpio_in(GPIO_BOARD_ID);
	battery_log(BAT_LOG_CRTI,"GPIO_BOARD_ID value is %d\n",board_gpio54);
	if(board_gpio54 == 0){
		batt_cust_data.v_charger_max = V_CHARGER_MAX_7;
	}
...
{
	unsigned int reset_status = true;
	battery_charging_control(CHARGING_CMD_SET_RESET, &reset_status);
...
static int __init battery_init(void)
{
	...
	/* m681 v148: UN-skipped (was battery/charger skip); needed for mt_usb_connect on VBUS -> USB/adb */
