...
extern int board_gpio54;
extern int g_set_charging_current;
...
static void set_jeita_charging_current(void)
{
	...
	if (g_temp_status == TEMP_NEG_10_TO_POS_0) {
		...
		g_temp_CC_value = CHARGE_CURRENT_800_00_MA; 	/* for low temp */
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA 0-10 set charging current : %d\r\n",
				    g_temp_CC_value);
	}
	else if (g_temp_status == TEMP_POS_45_TO_POS_60) {
		g_temp_CC_value = CHARGE_CURRENT_1000_00_MA;
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA 45-60 set charging current : %d\r\n",
				    g_temp_CC_value);
	}
}
...
unsigned int set_bat_charging_current_limit(int current_limit)
{
	...
	if (current_limit != -1) {
		...
		chr_type_ichg /= 100;
		if((BMT_status.charger_type != CHARGER_UNKNOWN) && (BMT_status.charger_type
				!= STANDARD_HOST) && ( BMT_status.charger_type != CHARGING_HOST)){
		if (current_limit > chr_type_ichg)
			current_limit = chr_type_ichg;
		}
        if (BMT_status.charger_type == STANDARD_HOST)
        {
            if (usb_current_set_flag)
                current_limit = CHARGE_CURRENT_800_00_MA/100;
        }//up to 800 input when usb current set
...
static unsigned int charging_full_check(void)
{
	...
	if (status == KAL_TRUE) {
		...
		if(bat_vol < BATTERY_VOLT_04_300000_V && (BMT_status.temperature > TEMP_POS_45_THRESHOLD || BMT_status.temperature < TEMP_POS_0_THRESHOLD)){
...
#endif
	if( g_set_charging_current > 0){
		if(g_set_charging_current < g_temp_CC_value){
			g_temp_CC_value = g_set_charging_current;
		}
	}
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
...
typedef enum {
    CHR_VDPM_VOLT_04_200000_V = 4200,
    CHR_VDPM_VOLT_04_280000_V = 4280,
    CHR_VDPM_VOLT_04_360000_V = 4360,
    CHR_VDPM_VOLT_04_440000_V = 4440,
    CHR_VDPM_VOLT_04_520000_V = 4520,
    CHR_VDPM_VOLT_04_600000_V = 4600,
    CHR_VDPM_VOLT_04_680000_V = 4680,
    CHR_VDPM_VOLT_04_760000_V = 4760,
    CHR_VDPM_VOLT_MAX,
} CHR_VDPM_ENUM;

int g_vol_status = VOL_ABOVE_4250;

#define VOL_ABOVE_4250_THRESHOLD  4250
#define VOL_ABOVE_4250_THRES_MINUS_X_DEGREE 4200

#define VOL_ABOVE_4000_THRESHOLD  4000
#define VOL_ABOVE_4000_THRES_MINUS_X_DEGREE 3950

#define VOL_ABOVE_3900_THRESHOLD  3900
#define VOL_ABOVE_3900_THRES_MINUS_X_DEGREE 3850

void charger_bat_vol_status (void)
{
    if (BMT_status.bat_vol > VOL_ABOVE_4250_THRESHOLD) {
        battery_log(BAT_LOG_CRTI, "[BATTERY][VDPM] bat voltage above (%d) !!\n\r", VOL_ABOVE_4250_THRESHOLD);
        g_vol_status = VOL_ABOVE_4250;
    } else if (BMT_status.bat_vol > VOL_ABOVE_4000_THRESHOLD) {
        if ((g_vol_status == VOL_ABOVE_4250) && (BMT_status.bat_vol >= VOL_ABOVE_4250_THRES_MINUS_X_DEGREE)) {
            battery_log(BAT_LOG_CRTI, "[BATTERY][VDPM] bat vol between %d and %d !!\n\r", VOL_ABOVE_4250_THRES_MINUS_X_DEGREE, VOL_ABOVE_4250_THRESHOLD);
        } else {
            battery_log(BAT_LOG_CRTI, "[BATTERY][VDPM] bat vol between %d and %d !!\n\r", VOL_ABOVE_4000_THRESHOLD, VOL_ABOVE_4250_THRESHOLD);
            g_vol_status = VOL_4000_TO_4250;
        }
    } else if (BMT_status.bat_vol > VOL_ABOVE_3900_THRESHOLD) {
        if ((g_vol_status == VOL_4000_TO_4250) && (BMT_status.bat_vol >= VOL_ABOVE_4000_THRES_MINUS_X_DEGREE)) {
            battery_log(BAT_LOG_CRTI, "[BATTERY][VDPM] bat vol between %d and %d !!\n\r", VOL_ABOVE_4000_THRES_MINUS_X_DEGREE, VOL_ABOVE_4000_THRESHOLD);
        } else {
            battery_log(BAT_LOG_CRTI, "[BATTERY][VDPM] bat vol between %d and %d !!\n\r", VOL_ABOVE_3900_THRESHOLD, VOL_ABOVE_4000_THRESHOLD);
            g_vol_status = VOL_3900_TO_4000;
        }
    } else {
        if ((g_vol_status == VOL_3900_TO_4000) && (BMT_status.bat_vol >= VOL_ABOVE_3900_THRES_MINUS_X_DEGREE)) {
            battery_log(BAT_LOG_CRTI, "[BATTERY][VDPM] bat vol between %d and %d !!\n\r", VOL_ABOVE_3900_THRES_MINUS_X_DEGREE, VOL_ABOVE_3900_THRESHOLD);
        } else {
            battery_log(BAT_LOG_CRTI, "[BATTERY][VDPM] bat vol below %d!!\n\r", VOL_ABOVE_3900_THRES_MINUS_X_DEGREE);
            g_vol_status = VOL_BELOW_3900;
        }
    }

}

static CHR_VDPM_ENUM select_vdpm_vol (void)
{
	CHR_VDPM_ENUM vdpm_vol = 0;
	unsigned short int register_value = 0;


	if (g_vol_status == VOL_ABOVE_4250) {
		vdpm_vol = CHR_VDPM_VOLT_04_600000_V;
	}
	else if (g_vol_status == VOL_4000_TO_4250) {
		vdpm_vol = CHR_VDPM_VOLT_04_520000_V;
	} else {
		vdpm_vol = CHR_VDPM_VOLT_04_360000_V;
}
	battery_log (BAT_LOG_CRTI,"%s board_gpio54 %d.\n", __func__, board_gpio54);
	if(board_gpio54 == 0){
		if(vdpm_vol == CHR_VDPM_VOLT_04_360000_V){
				register_value = 0x2;
				pmic_set_register_value(PMIC_RG_VCDT_LV_VTH, register_value);
		} else {
				register_value = 0x8;
				pmic_set_register_value(PMIC_RG_VCDT_LV_VTH, register_value);
		}
	...
	battery_log(BAT_LOG_CRTI, "[BATTERY][VDPM] register_value %d.\n", register_value);
	return vdpm_vol;
}

void charger_set_dynamic_vdpm (void)
{
    CHR_VDPM_ENUM chr_vdpm;
    charger_bat_vol_status ();
    chr_vdpm = select_vdpm_vol ();
    battery_log(BAT_LOG_CRTI, "[BATTERY][VDPM] charger_set_dynamic_vdpm %d.\n", chr_vdpm);
    battery_charging_control(CHARGING_CMD_SET_VINDPM, &chr_vdpm);
}
static void mtk_select_cv(void)
{
	...
	battery_log(BAT_LOG_CRTI,"[BATTERY] running test temperature:%d cv_voltage:%d\n",BMT_status.temperature, cv_voltage);
	#endif
	battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE, &cv_voltage);
...
static void pchr_turn_on_charging(void)
{
	...
	battery_log(BAT_LOG_CRTI,"[BATTERY] running test charger_type:%d  charging:%d  input current:%d\n",BMT_status.charger_type, g_temp_CC_value, g_temp_input_CC_value);
	#endif
