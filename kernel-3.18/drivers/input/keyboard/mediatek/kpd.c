...
#define INPUT_MINI_BUGREPORT_TRIGGER
#ifdef INPUT_MINI_BUGREPORT_TRIGGER
#define ANR_LOG_DELAY_TIME_4S	4
static struct hrtimer anr_log_timer_4s;
#endif
...
#ifdef INPUT_MINI_BUGREPORT_TRIGGER
static void anr_log_handler(u16 pressed)
{
    if (pressed)
    {
        hrtimer_start(&anr_log_timer_4s, ktime_set(ANR_LOG_DELAY_TIME_4S, 0), HRTIMER_MODE_REL);
    }
    else
    {
        hrtimer_cancel(&anr_log_timer_4s);
    }
}
#endif
...
#ifdef INPUT_MINI_BUGREPORT_TRIGGER
static enum hrtimer_restart anr_log_timer_4s_func(struct hrtimer *timer)
{
    if (!kpd_input_dev)
        return HRTIMER_NORESTART;

    input_report_key(kpd_input_dev, KEY_F23, 1);
    input_sync(kpd_input_dev);

    input_report_key(kpd_input_dev, KEY_F23, 0);
    input_sync(kpd_input_dev);
...
#ifdef INPUT_MINI_BUGREPORT_TRIGGER
    __set_bit(KEY_F23, kpd_input_dev->keybit);
#endif
...
#ifdef INPUT_MINI_BUGREPORT_TRIGGER
	__set_bit(KEY_F23, kpd_input_dev->evbit);
	hrtimer_init(&anr_log_timer_4s, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	anr_log_timer_4s.function = anr_log_timer_4s_func;
#endif
