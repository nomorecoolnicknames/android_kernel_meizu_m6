...
/* m681 ft5436 port: mt_eint_* not available in graft */
...
/* m681 ft5436 port: PMIC direct power functions defined in focaltech_driver.c */
extern void m681_touch_power_on(void);
extern void m681_touch_power_off(void);
    ...
    {
        {
            ...
            g_focalclient->addr = g_focal(client->addr & I2C_MASK_FLAG) | I2C_DMA_FLAG;

            if ((ret = i2c_master_send(g_focalclient, (char *)I2CDMABuf_pa, writelen)) != writelen)
            {
                FTS_DBG( "###%s i2c write len=%x\n", __func__, ret);
            }
            //MSE_ERR("Sensor dma timing is %x!\r\n", this_client->timing);
    ...
    {
        {
            ...
            g_focalclient->addr = g_focal(client->addr & I2C_MASK_FLAG) | I2C_DMA_FLAG;
            ret = i2c_master_recv(g_focalclient, (char *)I2CDMABuf_pa, readlen);
...
{
    /* m681 ft5436 port: use tpd_gpio_output (pinctrl) instead of mt_set_gpio_* */
...
int fts_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
    u8 reg_val[4] = {0};
    u32 i = 0;
    u8 __maybe_unused is_5336_new_bootloader = 0;
    u8 __maybe_unused is_5336_fwsize_30 = 0;
    u32 packet_number;
    u32 j = 0;
    u32 temp;
    u32 lenght;
...
int  fts_ctpm_fw_preupgrade_hwreset(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        ...
        {
            i_ret = 0x00;
            { u8 __reg_val = 0; fts_read_reg(client, 0xd0, &__reg_val); i_ret = __reg_val; }
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
    ...
    u8 __maybe_unused auc_i2c_write_buf;
...
static ssize_t ft5x0x_debug_write(struct file *filp, const char __user *buf,
...
static ssize_t ft5x0x_debug_read(struct file *filp, char __user *buf, size_t count,
                             loff_t *offp)
{
    struct i2c_client *client = PDE_DATA(file_inode(filp));
    int ret = 0;
    u8 __maybe_unused tx = 0, __maybe_unused rx = 0;
    int __maybe_unused i, __maybe_unused j;
    unsigned char tmp[FTS_PAGE_SIZE];
    ...
    { int __r = copy_to_user(buf, tmp, num_read_chars); (void)__r; }
