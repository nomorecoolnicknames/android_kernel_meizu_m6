...
/* m681 ft5436 port: upmu_set_rg_pwrkey_int_sel not available in graft */
...
/* m681 ft5436 port: graft kernel uses request_irq/disable_irq, not mt_eint_* */
static unsigned int fts_tp_irq = 0;
...
/* m681 ft5436 port: device tree provides i2c client, no board_info needed */

static const struct of_device_id ft5436_dt_match[] = {
	{.compatible = "mediatek,ft5436_touch"},
	{}
};
MODULE_DEVICE_TABLE(of, ft5436_dt_match);

static struct i2c_driver tpd_i2c_driver =
{
    .driver = {
        .name   = TPD_NAME,
        .of_match_table = ft5436_dt_match,
    },
    .probe      = tpd_probe,
    .remove     = tpd_remove,
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
    ...
    u8 reg;
    /* u8 report_rate = 0; unused — removed for graft port */
...
static void __maybe_unused tpd_gesture_report_key(u8 key1, u8 key2)
{
...
static struct device * __maybe_unused mx_tsp;   
...
/* m681 ft5436 port: upmu_is_chr_det not available in graft — stub to false */
static inline bool upmu_is_chr_det(void) { return false; }
...
static int __maybe_unused tpd_ioctl(unsigned int cmd)
{
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		if (ret > 0) {
			ret = -1;
			pr_err("[FORGE_TPD] tpd request_irq IRQ LINE NOT AVAILABLE!.\n");
		}
	} else {
		pr_err("[FORGE_TPD] tpd request_irq can not find touch eint device node!.\n");
		ret = -1;
	}
	pr_err("[FORGE_TPD] irq:%d\n", fts_tp_irq);
	return ret;
}
...
reset_proc:
    ...
    {
        pr_err("[FORGE_TPD] dma_buffer_init FAILED\n");
        return -1;
    }
    pr_err("[FORGE_TPD] dma_buffer_init OK\n");

    client->adapter->timeout = HZ / 10;
    pr_err("[FORGE_TPD] i2c read reg 0x00...\n");
    ...
    FTS_DBG("[FTS] tpd_probe irq_registration end...\r\n ");
...
//end
    ...
    { int __ret = sysfs_create_link(sysfs_get_devices_parent(), &client->dev.kobj, "mx_tsp"); (void)__ret; }
...
static int tpd_local_init(void)
{
    ...
    /* m681 ft5436 port: bypass regulator framework — use PMIC direct API.
     * CONFIG_MTK_LEGACY is not set, so hwPowerOn doesn't work.
     * The vldo28 regulator node doesn't match MT6351 hardware.
     * Direct pwrap write to MT6351 reg 0x0AA2 handles power. */
    tpd->reg = NULL;

    TPD_DMESG("FTS I2C Touchscreen Driver (ft5436 port)\n");
...
/* m681 ft5436 port: MZ_HALL_MODE and mz_tp_set_mode stubbed out */
static inline void mz_tp_set_mode(int mode) { }
