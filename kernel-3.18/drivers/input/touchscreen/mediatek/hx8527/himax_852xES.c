...
static u8 g_proximity_en = 0;
	...
	static unsigned char i_CTPM_FW_Truly[]=
	{
			...
			#include "Truly_L9830_LQ_C01_2017-03-10_1536.i"
			//#include "DJN_L9830_LQ_C30_2017-03-10_1550.i"
	};
	static unsigned char i_CTPM_FW_DJN[]=
	{
		//#include "LQ_L9830_DiJing_2017-03-16.i"
		#include "DJN_L9830_LQ_C31_2017-03-13_1847.i"
	};
#endif
#ifdef MTK
...
static int i_update_FW(void)
{
	    ...
	    if(0x11 == private_ts->vendor_sensor_id)
    {
      ...
      printk("himax,tpd_prob,hx8527e_ctp_modile_name = Truly");
    }
    else if(0x22 == private_ts->vendor_sensor_id)
    {
      ImageBuffer = i_CTPM_FW_DJN;
      fullFileLength = sizeof(i_CTPM_FW_DJN);
      printk("himax,tpd_prob,hx8527e_ctp_modile_name = DJN");
    }
    else
    {
      printk("\nNO compared Sensor ID,stop update firmware.\n");
      return 0;
    }
...
void himax_HW_reset(uint8_t loadconfig,uint8_t int_off)
{
	...
	if (ts->rst_gpio) {
		...
		printk("%s: yufangfang himax Now reset the Touch chip.\n", __func__);
...
static int touch_event_handler(void *ptr)
{
	...
	do
	{
		...
		printk("yufangfang himax interrupt\n");
		tpd_flag = 0;
...
int himax_ts_register_interrupt(struct i2c_client *client)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6755-touch");
	if (node) {
...
static int himax852xes_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct himax_ts_data *ts;
	struct himax_i2c_platform_data *pdata;
    printk("yufangfang hx8527 probe");
	//client->timing = 400;
