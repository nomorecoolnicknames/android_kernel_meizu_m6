...
#ifdef CONFIG_MMC_PASSWORDS
int mmc_devfreq_create_freq_table(struct mmc_host *host)
#else
...
#ifdef CONFIG_MMC_PASSWORDS
	if ((host->card->clk_scaling_lowest == 0) && (host->card->clk_scaling_highest == 0))
		goto out;
#endif
	...
	while (1) {
		...
		if (!atomic_read(&host->cq_rw)) {
			...
			if (dat_mrq) {
                ...
                {
					if(dat_mrq->data){
						if(dat_mrq->data->flags & MMC_DATA_READ){
							cmdq_read_blocks += dat_mrq->data->blocks;
                            printk("mlhxxx-%s:    blk_addr %d blksz %d blocks %d flags %08x tsac %lu ms nsac %d (r blocks %ld)\n",
                            mmc_hostname(host), blk_addr, dat_mrq->data->blksz,
                            dat_mrq->data->blocks, dat_mrq->data->flags,
                            dat_mrq->data->timeout_ns/NSEC_PER_MSEC,
                            dat_mrq->data->timeout_clks, cmdq_read_blocks);
                        }
                        if(dat_mrq->data->flags & MMC_DATA_WRITE){
                            cmdq_write_blocks += dat_mrq->data->blocks;
                            printk("mlhxxx-%s:    blk_addr %d blksz %d blocks %d flags %08x tsac %lu ms nsac %d (w blocks %ld)\n",
                            mmc_hostname(host), blk_addr, dat_mrq->data->blksz,
                            dat_mrq->data->blocks, dat_mrq->data->flags,
                            dat_mrq->data->timeout_ns/NSEC_PER_MSEC,
                            dat_mrq->data->timeout_clks, cmdq_write_blocks);
						}
					}
				}
#endif
...
#ifdef CONFIG_HUAWEI_EMMC_DSM
		if (mmc_card_mmc(card)){
			DSM_EMMC_LOG(card, DSM_EMMC_TUNING_ERROR,
