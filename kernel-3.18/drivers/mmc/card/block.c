...
{
	md->reset_done &= ~(unsigned int)type;
}
			       ...
			       struct mmc_queue *mq)
{
	...
	trace_mmc_blk_cmdq_rw_start(brq->data.blocks, req->tag, brq->que.arg,
	   (brq->data.blocks * brq->data.blksz));
#endif
...
static int mmc_blk_issue_rw_rq(struct mmc_queue *mq, struct request *rqc)
{
	...
	int ret = 1, disable_multi = 0, retry = 0, type=0, retune_retry_done = 0;
