...
int card_slot_status = 0;
static struct kobject *card_slot_kobj;
#define CAPACITY_2G             (2 * 1024 * 1024 * 1024ULL)
#ifdef CONFIG_HUAWEI_DSM
#include <dsm/dsm_pub.h>

static struct dsm_dev dsm_sdio = {
	.name = "dsm_sdio",
	.device_name = NULL,
	.ic_name = NULL,
	.module_name = NULL,
...
static struct dsm_client *dclient_sdio = NULL;
#endif
	...
	int async)
{
	...
	do {
		...
		if(host->hw->host_function == MSDC_SD)
		{
			...
			{
				sd_timeout_count ++;
				printk(KERN_ERR "[SD_FIX] Sdcard timeout happens, now count= %d \n ", sd_timeout_count);
				if(!(sd_timeout_count % 100))
				{
					sd_timeout_count = 0;
					return;
				}
			...
			{
				if(sd_timeout_count > 0)
				{
					sd_timeout_count --;
					printk(KERN_ERR "[SD_FIX]Not TMO, status = %d, will count down, now count= %d \n ", cmd->error, sd_timeout_count);
				}
			}
		}
...
static int msdc_ops_get_cd(struct mmc_host *mmc)
{
	...
	/* spin_unlock_irqrestore(&host->lock, flags); */
	card_slot_status = host->card_inserted;
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	host->mmc->slot_detect_change_flag = false;
#endif
	...
	*/
	...
	if (!dclient_sdio)
        {
            dclient_sdio = dsm_register_client(&dsm_sdio);
       }
#endif
	return 0;
...
/* module init/exit                                                         */
/*--------------------------------------------------------------------------*/
static ssize_t card_slot_status_show(struct kobject *kobj, struct kobj_attribute *attr,
char *buf)
{
    return sprintf(buf, "%d\n", card_slot_status);
}

struct kobj_attribute card_slot_status_attr = {
    .attr = {"card_slot_status", 0660},
    .show = &card_slot_status_show,
};

static struct attribute * card_slot_attr[] = {
    &card_slot_status_attr.attr,
    NULL,
};

static struct attribute_group card_slot_group = {
.attrs = card_slot_attr,
};
static int __init mt_msdc_init(void)
{
	...
	card_slot_kobj = kobject_create_and_add("card_slot", NULL);
	if (card_slot_kobj)
	rc=sysfs_create_group(card_slot_kobj, &card_slot_group);
	if(rc){
		printk("create card slot status sys files err\n");
		return 0;
	}
	return 0;
}
...
#ifdef CONFIG_HUAWEI_DSM
void msdc_sdio_dsm_dump(struct mmc_host *host, struct mmc_command *cmd, int err_num)
{
	if (dclient_sdio == NULL) {
	...
	if (host == NULL ) {
       ...
       {
            printk(KERN_ERR "dsm_sido Host[%s], index: %d, err_num = %d \n", mmc_hostname(host), host->index, err_num);
            if (!dsm_client_ocuppy(dclient_sdio))
            {
                    dsm_client_record(dclient_sdio, "Host[%s], index: %d\n", mmc_hostname(host) , host->index);
                    dsm_client_record(dclient_sdio, "f_min = %d, f_max = %d, f_init = %d\n", host->f_min, host->f_max, host->f_init);
                    dsm_client_record(dclient_sdio, "ocr_avail = 0x%x, ocr_avail_sdio = 0x%x\n", host->ocr_avail, host->ocr_avail_sdio);
                    dsm_client_record(dclient_sdio, "caps = 0x%x, caps2 = 0x%x, pm_caps = 0x%x\n", host->caps, host->caps2, host->pm_caps);
                    dsm_client_record(dclient_sdio, "sdio_irqs = %d, sdio_irq_pending = %d\n", host->sdio_irqs, host->sdio_irq_pending);
                    dsm_client_record(dclient_sdio, "max_seg_size = %d, max_segs = %d, max_req_size = %d, max_blk_size = %d, max_blk_count = %d, max_busy_timeout = %d\n",
                                        host->max_seg_size, host->max_segs, host->max_req_size, host->max_blk_size, host->max_blk_count, host->max_busy_timeout);
                    if(NULL != cmd)
                    {
                            dsm_client_record(dclient_sdio, "Cmd[0x%x], with arg 0x%x\n", cmd->opcode, cmd->arg);
                            dsm_client_record(dclient_sdio, "resp_0 = 0x%x, resp_1 = 0x%x, resp_2 = 0x%x, resp_3 = 0x%x, flags = 0x%x\n",
                                                cmd->resp[0], cmd->resp[1], cmd->resp[2], cmd->resp[3], cmd->flags);
                            dsm_client_record(dclient_sdio, "retries = %d, error = %d, busy_timeout = %d \n", cmd->retries, cmd->error, cmd->busy_timeout);
                     }
                    else
                    {
                            dsm_client_record(dclient_sdio, "Cmd is empty!\n");
                     }
                    dsm_client_notify(dclient_sdio, err_num);
            }
            else
            {
                printk("DSM CALL FAIL, Host[%s],  index: %d\n", mmc_hostname(host), host->index );
            }
        }
}

EXPORT_SYMBOL(msdc_sdio_dsm_dump);
#endif
