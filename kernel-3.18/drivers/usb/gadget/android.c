...
{
//copy from dehli
	//f->config = kzalloc(sizeof(struct functionfs_config), GFP_KERNEL);// modify by zhaofei - 2017-02-23-16-18
	f->config = kzalloc(sizeof(struct functionfs_hdb_config), GFP_KERNEL);
	if (!f->config) {
		return -ENOMEM;
	}
