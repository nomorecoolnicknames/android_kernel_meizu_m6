...
#define SYNAPTICS_VENDER_NAME  "synaptics-S3603R"
#define EASY_WAKEUP_FASTRATE 0x011D
#define F54_ANALOG_CMD0  0x0141
#define PALM_REG_BIT 0x01
#define F12_OBJECT_REPORT_ENABLE  0x001C
...
static char version[]="s3603 ver 1.0";
static char vendor_name[50]="oflim-s3603,";
...
#define SYNAPTICS_FW_UPGRADE_DELAY	 "fw_upgrade_delay"
...
{	
	...
	retval =
	    of_property_read_u32(device, SYNAPTICS_FW_UPGRADE_DELAY,
				 &chip_data->fw_upgrade_delay);
	if (!retval) {
		TS_LOG_INFO("get fw upgrade delay = %d\n",
			    chip_data->fw_upgrade_delay);
	}
...
static int synaptics_power_init(void)
{
    ...
    /*set TP volt*/
    retval = regulator_set_voltage(
        rmi4_data->synaptics_chip_data->ts_platform_data->vdd,
        2800000,2800000);
    if (retval != 0)
    {
    ...
    retval = regulator_enable(rmi4_data->synaptics_chip_data->
        ts_platform_data->vdd);
    if (retval != 0)
    {
...
void synaptics_power_suspend(void)
{
#if 0
    int retval;

    retval = regulator_disable(rmi4_data->synaptics_chip_data->
        ts_platform_data->vdd);
    if (retval != 0)
        TS_LOG_ERR("[%s]Failed to disable regulator when suspend ret=%d!",
            __func__, retval);
#endif
}

int synaptics_power_resume(void)
{
    int retval = 0;
#if 0
    retval = regulator_enable(rmi4_data->synaptics_chip_data->
        ts_platform_data->vdd);
    if (retval != 0)
        TS_LOG_ERR("[%s]Failed to enable regulator when resume,ret=%d!",
            __func__,retval);
#endif
    return retval;
}
...
static void synaptics_hardware_reset_chip(void)
{
    TS_LOG_INFO("[%s]:synaptics_hardware_reset\n",__func__);
    //tpd_gpio_as_int(1);
    tpd_gpio_output(GTP_RST_PORT, 1);// reset pin high
    msleep(200);
    tpd_gpio_output(GTP_RST_PORT, 0);// reset pin low
    msleep(20);
    tpd_gpio_output(GTP_RST_PORT, 1);// reset pin high
    msleep(200);
}
...
data_release:
	...
	strcat(vendor_name,config_id_string);
...
{
	int retval = NO_ERR;
	unsigned char config_id_str[CHIP_INFO_LENGTH] = { 0 };
	...
	strncpy(config_id_str,rmi4_data->rmi4_mod_info.device_config_id,
		strlen(rmi4_data->rmi4_mod_info.device_config_id));
	memset(vendor_name,0,sizeof(vendor_name));
	strncpy(vendor_name,"oflim-s3603,",strlen("oflim-s3603,"));
	strcat(vendor_name,config_id_str);
	TS_LOG_INFO("synaptics_fw_update_sd .config_id_string=%s.vendor_name=%s\n",
		config_id_str,vendor_name);
	synaptics_rmi4_status_resume(rmi4_data);
data_release:
	if (SYNAPTICS_S3718 != rmi4_data->synaptics_chip_data->ic_type
		&& SYNAPTICS_TD4322 != rmi4_data->synaptics_chip_data->ic_type
	...
	TS_LOG_INFO("synaptics_fw_update_sd end.\n");
	return retval;
}
...
static int synaptics_set_glove_switch(u8 glove_switch)
{
	...
	retval =
		synaptics_rmi4_i2c_read(rmi4_data, F12_OBJECT_REPORT_ENABLE,
			(unsigned char *)&glove_temp_value, 1);
	if (retval < 0) {
		TS_LOG_ERR("read glove  f12 information failed: %d\n", retval);
		goto out;
	}
	if (glove_switch) {
		glove_temp_value |= 1 << 5;
	} else {
		glove_temp_value &= ~(1 << 5);
	}

	retval =
		synaptics_rmi4_i2c_write(rmi4_data, F12_OBJECT_REPORT_ENABLE,
			(unsigned char *)&glove_temp_value, 1);
	if (retval < 0) {
		TS_LOG_ERR("write glove f12 information failed: %d\n", retval);
	}
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		retval =
			synaptics_rmi4_i2c_write(rmi4_data, F54_ANALOG_CMD0,
						&device_ctrl, sizeof(device_ctrl));
		if (retval < 0) {
			TS_LOG_ERR("Failed to set enable gesture\n");
			return;
		} else {
			TS_LOG_ERR("succ to set enable gesture\n");
		}
		...
		while(loop--)
		{
			...
			retval =
				synaptics_rmi4_i2c_read(rmi4_data, F54_ANALOG_CMD0,
							&device_ctrl, sizeof(device_ctrl));
			if (retval < 0) {
				TS_LOG_ERR("Failed to read enable gesture\n");
			} else {
				//TS_LOG_ERR("succ to read enable gesture, device_ctrl=0x%x\n",device_ctrl);
				if(!device_ctrl)
				{
					TS_LOG_INFO("device_ctrl = 0 succ,break;\n");
					break;
				}
			}
		}
		...
		if(!loop)
		{
			TS_LOG_ERR("read enable gesture timeout.\n");
		}
		...
		if (retval < 0) {
			...
			TS_LOG_INFO
				("write ok,then read back,read valve(0x%02x)\n", device_ctrl_data[2]);
		}
	}
...
static void synaptics_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!strcmp(rmi4_data->synaptics_chip_data->ts_platform_data->product_name, "jimmy"))
	{
		TS_LOG_ERR("find jimmy\n");
		device_ctrl = (device_ctrl & ~MASK_2BIT);
		device_ctrl =
		    (device_ctrl | SENSOR_SLEEP_BIT0);
	}
	else {
		device_ctrl = (device_ctrl & ~MASK_2BIT);
		device_ctrl =
...
static int synaptics_suspend(void)
{
	...
	case TS_GESTURE_MODE:
		...
		enable_irq_wake(rmi4_data->synaptics_chip_data->ts_platform_data->irq_id);
				    ...
				    unsigned short length)
{
	...
	retval =
#if 1
		rmi4_data->synaptics_chip_data->ts_platform_data->bops->
			bus_write(&reg_addr,1,data, length,3);
		printk("[%s]:reg_addr = 0x%x,write_val = 0x%x\n",
			__func__,reg_addr,*(data));
#else
		rmi4_data->synaptics_chip_data->ts_platform_data->bops->
			bus_write(wr_buf, length + 1);
#endif
	if (retval < 0) {
				   ...
				   unsigned int address)
{
	...
	if (page != rmi4_data->current_page) {
		...
		retval =
#if 1
		rmi4_data->synaptics_chip_data->ts_platform_data->bops->bus_write(buf,1,buf+1,
			PAGE_SELECT_LEN-1,3);
#else
