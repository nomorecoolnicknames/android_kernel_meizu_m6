...
static bool charger_switch_state = 0;
...
int goodix_sleep_mode_out(void)
{
	...
	if(charger_switch_state)
	goodix_send_cmd(0x8040,GTP_CMD_CHARGER_ON);
	else
	goodix_send_cmd(0x8040,GTP_CMD_CHARGER_OFF);
	ts->enter_resume = true;
...
static int goodix_chip_init(void)
{
	struct goodix_ts_data *ts = goodix_ts;
	int ret = -1;
	ret = goodix_chip_parse_config(ts->goodix_device_data->ts_platform_data->node, ts ->goodix_device_data);
	if (ret < 0)
	{
	...
	ret = goodix_parse_dts(ts->goodix_device_data->ts_platform_data->node,ts);
	if (ret < 0)
		return ret;
	if (ts->tools_support)
		init_wr_node();
...
static int goodix_chip_get_info(struct ts_chip_info_param *info)
{
	...
	sprintf(info->fw_vendor, "FW ver: %02X%02X",(ts->hw_info.patch_id>>8)&0xFF,(ts->hw_info.patch_id)&0xFF);
...
static int goodix_esdcheck_func(void)
{
	...
	if(false == ts->enter_resume)
	{
		TS_LOG_INFO("ESD NOT READY!");
		return ret;
	}
...
static int goodix_charger_switch(struct ts_charger_info *info)
{
	...
	case TS_ACTION_WRITE:
		...
		{
			if (!goodix_ts->enter_suspend)
			ret = goodix_send_cmd(0x8040,GTP_CMD_CHARGER_ON);
			charger_switch_state = 1;
			TS_LOG_INFO("Charger cmd switch on");
		}
		else
		{
			if (!goodix_ts->enter_suspend)
			ret = goodix_send_cmd(0x8040,GTP_CMD_CHARGER_OFF);
			charger_switch_state = 0;
	...
	goodix_chip_detect(struct ts_kit_platform_data *pdata)
{
	...
	goodix_ts->pdev->dev.of_node = pdata->node;
	...
	//goodix_parse_dts(pdata->node,goodix_ts);
	ret = goodix_chip_reset();
	if (ret < 0)
		goto err_power_on;
	ret = goodix_i2c_test();
