...
ssize_t goodix_tool_write(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
    ...
    if (1 == cmd_head.wr)
    {
        if(cmd_head.addr_len > GTP_ADDR_LENGTH)
                return -EPERM;
	//if(cmd_head.data_len > strlen(&cmd_head.data[GTP_ADDR_LENGTH]))
	if(cmd_head.data_len > (512*5-2))
		return -EPERM;
	    ...
	    //if(cmd_head.addr_len > strlen(&cmd_head.data[GTP_ADDR_LENGTH - cmd_head.addr_len]))
		//if(cmd_head.addr_len > (512*5))
		    //return -EPERM;
    ...
    {
        //if(cmd_head.data_len > strlen(&cmd_head.data[0]))
		if(cmd_head.data_len > (512*5))
		return -EPERM;
