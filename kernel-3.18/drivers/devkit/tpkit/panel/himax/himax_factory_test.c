...
#define HX_RAW_DUMP_FILE "/data/log/tp_self_test.txt"
...
#ifdef HX_FAC_LOG_PRINT
	/*=====debug log======*/
	TS_LOG_INFO("mutual bank start\n");
	for (index1 = 0; index1 < rx *tx; index1++)
	{
		if(index1%rx == 0) printk("\n");
		printk("%d ",mutual_bank[index1]);
	}
	TS_LOG_INFO("self bank start\n");
	for(index1 = 0; index1 < rx+tx; index1++)
	{
		if(index1%rx == 0) printk("\n");
		printk("%d ",self_bank[index1]);
	}
	/*=====debug log=====*/
#endif
...
int himax_factory_start(struct himax_ts_data *ts,struct ts_rawdata_info *info_top)
{
	...
	fn = filp_open(HX_RAW_DUMP_FILE,O_CREAT | O_RDWR ,0);
	if (!IS_ERR(fn))
	{
...
static void save_rawrata_file(int fd,uint8_t *buf,int rx,int tx)
{
	int len = rx*tx*4*sizeof(uint8_t)+(rx+tx)*4*sizeof(uint8_t)+tx*sizeof(uint8_t);
...
int himax_factory_start(struct himax_ts_data *ts,struct ts_rawdata_info *info_top)
{
	...
	fac_dump_step += 2;
	himax_fac_dump(fac_dump_step,mutual_num-rx,mutual_num-tx,rx_delta,tx_delta);
	fac_dump_step += 2;
	/* step3: Noise Delta */
	...
	fs = get_fs();
	set_fs(KERNEL_DS);
	fd = sys_open(HX_RAW_DUMP_FILE,O_CREAT | O_RDWR ,0666);
	if (fd>=0)
	{
		TS_LOG_INFO("%s: fac write file \n",__func__);
		save_rawrata_file(fd,fac_dump_buffer,rx,tx);
		sys_close(fd);
		set_fs(fs);
	}
	else
