...
char g_fts_vendor_name[50]="each-ft5436";
...
int focal_write(u8 *values, u16 values_size)
{
	...
	bops = g_focal_dev_data->ts_platform_data->bops;
    if(values_size == 1) {
        ret = bops->bus_write(values,1, &default_zero, 1,3);
    }
    else {
...
void focal_power_suspend(void)
{
    int retval;

    retval = regulator_disable(g_focal_dev_data->ts_platform_data->vdd);
    if (retval != 0)
        TS_LOG_ERR("[%s]Failed to disable regulator when suspend ret=%d!",__func__, retval);
}

int focal_power_resume(void)
{
    int retval = 0;

    retval = regulator_enable(g_focal_dev_data->ts_platform_data->vdd);
    if (retval != 0)
        TS_LOG_ERR("[%s]Failed to enable regulator when resume,ret=%d!", __func__,retval);
