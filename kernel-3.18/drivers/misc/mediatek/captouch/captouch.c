...
#if 0//defined(CONFIG_CAPSENSOR_SX931X)
#include <linux/fb.h>
...
static ssize_t cap_store_delay(struct device *dev, struct device_attribute *attr,
                  const char *buf, size_t count)
{
    return count;
}

static ssize_t cap_show_delay(struct device *dev,
                 struct device_attribute *attr, char *buf)
{
    int len = 0;

    CAPTOUCH_LOG(" not support now\n");
    return len;
}


static ssize_t cap_store_batch(struct device *dev, struct device_attribute *attr,
                  const char *buf, size_t count)
{
    CAPTOUCH_LOG(" cap_store_batch not supported\n");
    return count;
}

static ssize_t cap_show_batch(struct device *dev,
                 struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "%d\n", 0);
}

static ssize_t cap_store_flush(struct device *dev, struct device_attribute *attr,
                  const char *buf, size_t count)
{
    return count;
}

static ssize_t cap_show_flush(struct device *dev,
                 struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "%d\n", 0);
}

static ssize_t cap_show_devnum(struct device *dev,
                 struct device_attribute *attr, char *buf)
{
    unsigned int devnum;
    ...
    list_for_each_entry(handle, &captouch_context_obj->idev->h_list, d_node)
        if (strncmp(handle->name, "event", EVENT_LENGTH) == 0) {
            devname = handle->name;
            break;
        }

    ret = sscanf(devname+EVENT_LENGTH, "%d", &devnum);
    return snprintf(buf, PAGE_SIZE, "%d\n", devnum);
}
