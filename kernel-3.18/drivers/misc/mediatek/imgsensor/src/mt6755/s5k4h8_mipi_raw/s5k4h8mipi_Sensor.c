...
#define LOG_DEBUG(format, args...)  pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    spin_unlock(&imgsensor_drv_lock);
   // printk("gain = %d , reg_gain = 0x%x ", gain, reg_gain);
...
static void preview_setting(void)
{
    ...
    else //if(chip_id == 0xc001)
    {
		...
		write_cmos_sensor(0x0B00, 0x0180);//open lsc shading
		write_cmos_sensor(0x6028, 0x2000);
...
/*jin.wang add camera otp calibration start*/
static int CheckOtp = 0x00;
struct OTP_STRUCT {
    int module_flag;
    int awb_flag;
    int lsc_flag;
    ...
    int gray_width_high;
    int gray_width_low;
    int gray_height_high;
    int gray_height_low;
    int module_info_checksum;
    int lsc_info_checksum;
    int awb_data[64];
    int lsc_data[1024];
};

struct OTP_STRUCT S5K4H8_OTP_STRUCT;

/*jijin.wang add section page & enable read page func*/
static void section_page_start_read(UINT32 Page){
    write_cmos_sensor(0x0100,0x0100);
    write_cmos_sensor_8(0x0A02,Page);//section page(0~F)
    write_cmos_sensor_8(0x0A00,0x01);//Enter read mode
    mdelay(5);
}

/*jijin.wang add disable read func*/
static void disable_read_func(void){
    write_cmos_sensor_8(0x0A00,0x00);//Disable read mode
    write_cmos_sensor(0x0100,0x0000);
}

static int Read_CamOtpInfo_CheckSum(void){
    /*read module flag & checksum module info start*/
    int module_addr = 0x00;
    //int lsc_shading_addr = 0x00;
    int awb_info_addr = 0x00;
    int Module_CheckSum = 0x00;
    int Awb_CheckSum = 0x00;
    int Lsc_CheckSum = 0x00;
    int ReadI = 0x00;
    int Page = 0x00;
    //memset(&S5K4H8_OTP_STRUCT,0, sizeof(S5K4H8_OTP_STRUCT));
    section_page_start_read(0xF);
    S5K4H8_OTP_STRUCT.module_flag = read_cmos_sensor_8(0x0A06);
    //LOG_INF("module_flag = %d\n",S5K4H8_OTP_STRUCT.module_flag);
    if(S5K4H8_OTP_STRUCT.module_flag == 0x01)
    {
        module_addr = 0x0A07;
    }else if(S5K4H8_OTP_STRUCT.module_flag == 0x13){
        module_addr = 0x0A12;
    }else{
        LOG_INF("read otp module flag fail!!!,close otp clibration\n");
        return -1;
    }
    S5K4H8_OTP_STRUCT.module_id = read_cmos_sensor_8(module_addr++);
    S5K4H8_OTP_STRUCT.lens_id = read_cmos_sensor_8(module_addr++);
    S5K4H8_OTP_STRUCT.module_year = read_cmos_sensor_8(module_addr++);
    S5K4H8_OTP_STRUCT.module_month = read_cmos_sensor_8(module_addr++);
    S5K4H8_OTP_STRUCT.module_day = read_cmos_sensor_8(module_addr++);
    S5K4H8_OTP_STRUCT.glass_id = read_cmos_sensor_8(module_addr++);
    S5K4H8_OTP_STRUCT.gray_width_high = read_cmos_sensor_8(module_addr++);
    S5K4H8_OTP_STRUCT.gray_width_low = read_cmos_sensor_8(module_addr++);
    S5K4H8_OTP_STRUCT.gray_height_high = read_cmos_sensor_8(module_addr++);
    S5K4H8_OTP_STRUCT.gray_height_low = read_cmos_sensor_8(module_addr++);
    S5K4H8_OTP_STRUCT.module_info_checksum = read_cmos_sensor_8(module_addr);
    //LOG_INF("module_id = %d,lens_id = %d,module_year = %d,module_month = %d,module_day = %d,glass_id = %d,gray_width_high = %d,gray_width_low = %d,gray_height_high = %d,gray_height_low = %d,module_info_checksum = %d\n",S5K4H8_OTP_STRUCT.module_id,S5K4H8_OTP_STRUCT.lens_id,S5K4H8_OTP_STRUCT.module_year,S5K4H8_OTP_STRUCT.module_month,S5K4H8_OTP_STRUCT.module_day,S5K4H8_OTP_STRUCT.glass_id,S5K4H8_OTP_STRUCT.gray_width_high,S5K4H8_OTP_STRUCT.gray_width_low,S5K4H8_OTP_STRUCT.gray_height_high,S5K4H8_OTP_STRUCT.gray_height_low,S5K4H8_OTP_STRUCT.module_info_checksum);
    /*check module info is or not is right */
    Module_CheckSum = S5K4H8_OTP_STRUCT.module_id + S5K4H8_OTP_STRUCT.lens_id + S5K4H8_OTP_STRUCT.module_year
                    + S5K4H8_OTP_STRUCT.module_month + S5K4H8_OTP_STRUCT.module_day + S5K4H8_OTP_STRUCT.glass_id + S5K4H8_OTP_STRUCT.gray_width_high +
                    + S5K4H8_OTP_STRUCT.gray_width_low + S5K4H8_OTP_STRUCT.gray_height_high + S5K4H8_OTP_STRUCT.gray_height_low;
    if(((Module_CheckSum % 255) +1) == S5K4H8_OTP_STRUCT.module_info_checksum)
    {
        LOG_INF("module info check success!!!\n");
        CheckOtp = 0x001;
    }else{
        LOG_INF("module info check fail!!!\n");
        return -1;
    }
    /*read module flag & checksum module info end*/
    /*read awb info & checksum */
    S5K4H8_OTP_STRUCT.awb_flag = read_cmos_sensor_8(0xA1D);
    if(S5K4H8_OTP_STRUCT.awb_flag == 0x01){
        awb_info_addr = 0x0A1E;
    }else if(S5K4H8_OTP_STRUCT.awb_flag == 0x13){
        awb_info_addr = 0x0A27;
    }else{
        LOG_INF("read awb flag fail!!!\n");
        return -1;
    }
    for(ReadI = 0x00;ReadI <= 8;ReadI++)
    {
        S5K4H8_OTP_STRUCT.awb_data[ReadI] = read_cmos_sensor_8(awb_info_addr++);
        LOG_INF("awb_data[0x%x] = 0x%x\n",awb_info_addr - 1,S5K4H8_OTP_STRUCT.awb_data[ReadI]);
        if(ReadI <= 7){
            Awb_CheckSum += S5K4H8_OTP_STRUCT.awb_data[ReadI];
        }else{
            Awb_CheckSum = Awb_CheckSum % 255 + 1;
            LOG_INF("Awb_Checksum = 0x%x\n",Awb_CheckSum);
        }
    }
    /*checksum awb info is or not is right*/
    if(Awb_CheckSum == S5K4H8_OTP_STRUCT.awb_data[8])
    {
        LOG_INF("Awb info check success!!!!!\n");
        CheckOtp = CheckOtp | 0x010;
    }else{
        LOG_INF("awb info check fail!!!!\n");
        return -1;
    }
    /*disable page15 read able*/
    /*read lsc shding checksum*/
    S5K4H8_OTP_STRUCT.lsc_info_checksum = read_cmos_sensor_8(0x0A04);
    //LOG_INF("lsc_info_checksum = 0x%x\n",S5K4H8_OTP_STRUCT.lsc_info_checksum);
    disable_read_func();
    /*read lsc shading & lsc_shading checksum*/
    /*section page5 ~ 10*/
    for(Page = 5;Page <= 10;Page++){
        section_page_start_read(Page);
        if(Page == 5){
            for(ReadI = 0x0A1F;ReadI <= 0x0A43;ReadI++)
            {
                S5K4H8_OTP_STRUCT.lsc_data[ReadI - 0x0A1F] = read_cmos_sensor_8(ReadI);
                Lsc_CheckSum += S5K4H8_OTP_STRUCT.lsc_data[ReadI - 0x0A1F];
            }
        }else if(Page == 10){
            for(ReadI = 0x0A04;ReadI <= 0x0A10;ReadI++)
            {
                S5K4H8_OTP_STRUCT.lsc_data[37 + ((Page - 6)* 64) + ReadI - 0x0A04] = read_cmos_sensor_8(ReadI);
                Lsc_CheckSum += S5K4H8_OTP_STRUCT.lsc_data[37 + ((Page - 6)* 64) + ReadI - 0x0A04];
            }
        }else{
            for(ReadI = 0x0A04;ReadI <= 0x0A43;ReadI++)
            {
                S5K4H8_OTP_STRUCT.lsc_data[37 + ((Page - 6)* 64) + ReadI - 0x0A04] = read_cmos_sensor_8(ReadI);
                Lsc_CheckSum += S5K4H8_OTP_STRUCT.lsc_data[37 + ((Page - 6)* 64) + ReadI - 0x0A04];
            }
    ...
    Lsc_CheckSum = Lsc_CheckSum % 255 + 1;
    //LOG_INF("Lsc_CheckSum = 0x%x\n",Lsc_CheckSum);
    /*check lsc shading is or not is right*/
    if(Lsc_CheckSum == S5K4H8_OTP_STRUCT.lsc_info_checksum)
    {
        LOG_INF("Lsc info checksum success!!!\n");
        CheckOtp = CheckOtp | 0x100;
    }else{
        LOG_INF("Lsc info checksum fail!!!\n");
        return -1;
    }
    //LOG_INF("CheckOtp = 0x%x\n",CheckOtp);
    return CheckOtp;
}

/*awb calibration apply*/
static int Awb_Calibration_Apply(void){
    kal_uint16 r_ratio, b_ratio;
	kal_uint16 r_gain , g_gain, b_gain;
	kal_uint16 gr_gain, gb_gain;
	int rg = 0x00,bg = 0x00,golden_rg = 0x00,golden_bg = 0x00;
	static kal_uint16 GAIN_DEFAULT = 0x0100;
	rg = ((S5K4H8_OTP_STRUCT.awb_data[0]) * 1024)/((S5K4H8_OTP_STRUCT.awb_data[2] + S5K4H8_OTP_STRUCT.awb_data[3])/2);
	bg = ((S5K4H8_OTP_STRUCT.awb_data[1]) * 1024)/((S5K4H8_OTP_STRUCT.awb_data[2] + S5K4H8_OTP_STRUCT.awb_data[3])/2);
	golden_rg = 0x216;//((S5K4H8_OTP_STRUCT.awb_data[4]) * 1024)/((S5K4H8_OTP_STRUCT.awb_data[6] + S5K4H8_OTP_STRUCT.awb_data[7])/2);
	golden_bg = 0x255;//((S5K4H8_OTP_STRUCT.awb_data[5]) * 1024)/((S5K4H8_OTP_STRUCT.awb_data[6] + S5K4H8_OTP_STRUCT.awb_data[7])/2);
	//LOG_INF("rg = 0x%x,bg = 0x%x,golden_rg = 0x%x,golden_bg = 0x%x\n",rg,bg,golden_rg,golden_bg);
	if((rg == 0)||(bg == 0))
	{
	...
	//LOG_INF("g_gain = 0x%x,r_gain = 0x%x,b_gain = 0x%x,g_gain = 0x%x\n",read_cmos_sensor(0x020e),read_cmos_sensor(0x0210),read_cmos_sensor(0x0212),read_cmos_sensor(0x0214));
    return 1;
}

static void Lsc_Calibration_Apply(void){
    LOG_INF("open lsc shading in setting\n");
}

/*apply read otp data from sensor*/
static void Apply_Otp_Calibration(void){
    /*lsc shading calibration apply*/
    Lsc_Calibration_Apply();
    /*awb calibration apply*/
    Awb_Calibration_Apply();
}

static int Otp_Calibration(void){
    /*add read camera info & checksum func*/
    if(CheckOtp == 0x111){
        Apply_Otp_Calibration();
    }else{
        if(Read_CamOtpInfo_CheckSum() != 0x111){
            LOG_INF("read otp fail;close otp calibration\n");
        }else{
            LOG_INF("read otp data success,apply awb & lsc shading\n");
            Apply_Otp_Calibration();
        }
    }
    return CheckOtp;
    /*add awb calibration func*/
}
/*jin.wang add camera otp calibration end*/
/*************************************************************************
* FUNCTION
*	get_imgsensor_id
*
...
static kal_uint32 open(void)
{
    ...
    if(Otp_Calibration() == 0x111)
    {
        if(OTPALLCHECK == 0 || OTPALLCHECK == 2){
