...
static imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0x78779e1c,//modify by wangjijin
	.pre = {
...
#else
    { 4256, 3168, 816, 848, 2624, 1472, 1312,  736,  8,        8,      1280,       720, 0,   0,    1280,  720},//hs-video
#endif
...
{
	kal_uint16 BaseLineTime =0x25a8; //9643 line
	if(shutter > 0x7148) //300ms
	{
		kal_uint32 tempValue = shutter/BaseLineTime;	
		LOG_INF("tempValue = %d\n",tempValue);
   		switch (tempValue){
			case 4:
				LOG_INF("400ms shutter =%d\n", shutter);
				write_cmos_sensor(0x380c,0x08);
				write_cmos_sensor(0x380d,0xc4);
				write_cmos_sensor(0x380e,0x4b);
				write_cmos_sensor(0x380f,0x3c);
				write_cmos_sensor(0x3500,0x04);
				write_cmos_sensor(0x3501,0xb3);
				write_cmos_sensor(0x3502,0x30);
				break;
			case 5:
				LOG_INF("500ms shutter =%d\n", shutter);
				write_cmos_sensor(0x380c,0x08);
				write_cmos_sensor(0x380d,0xc4);
				write_cmos_sensor(0x380e,0x5e);
				write_cmos_sensor(0x380f,0x09);
				write_cmos_sensor(0x3500,0x05);
				write_cmos_sensor(0x3501,0xe0);
				write_cmos_sensor(0x3502,0x00);
				break;
			case 6:
				LOG_INF("600ms shutter =%d\n", shutter);
				write_cmos_sensor(0x380c,0x08);
				write_cmos_sensor(0x380d,0xc4);
				write_cmos_sensor(0x380e,0x70);
				write_cmos_sensor(0x380f,0xd6);
				write_cmos_sensor(0x3500,0x07);
				write_cmos_sensor(0x3501,0x0c);
				write_cmos_sensor(0x3502,0xd0);
				break;
			default:
				LOG_INF(" default shutter =%d\n", shutter);
                //shutter = 0x3d0;
				break;
		}
...
static void sensor_init(void)
{
	...
	mdelay(10);
	write_cmos_sensor(0x0300, 0x07);//02 modify mipi clck request 135->137.5 270->275 540->550 actual 135->137.625 270->275.25 540->550.5 
	write_cmos_sensor(0x0301, 0x01);//00 modify mipi clck request 135->137.5 270->275 540->550 actual 135->137.625 270->275.25 540->550.5
	write_cmos_sensor(0x0302, 0x6f);//5a modify mipi clck request 135->137.5 270->275 540->550 actual 135->137.625 270->275.25 540->550.5 
...
static void preview_setting(void)
{
    ...
    write_cmos_sensor(0x3800, 0x00);//0705_rewrite initial value
	write_cmos_sensor(0x3801, 0x00);//s_x=0x00
	
	write_cmos_sensor(0x3802, 0x00);
	write_cmos_sensor(0x3803, 0x08);//x_y=0x08
    write_cmos_sensor(0x3804, 0x10);//0705_rewrite initial value
	...
	write_cmos_sensor(0x3813, 0x04);//y_offset=4

	
	write_cmos_sensor(0x3814, 0x03);
    write_cmos_sensor(0x3815, 0x01);//0705_rewrite initial value
	write_cmos_sensor(0x3816, 0x03);
    write_cmos_sensor(0x3817, 0x01);//0705_rewrite initial value
	...
	write_cmos_sensor(0x4837, 0x1d);//1c->1d modify mipi clck
...
if(currefps == 150){
    ...
    write_cmos_sensor(0x3800, 0x00);//0705rewrite initial value
	write_cmos_sensor(0x3801, 0x00);
	write_cmos_sensor(0x3802, 0x00);
	write_cmos_sensor(0x3803, 0x08);
    write_cmos_sensor(0x3804, 0x10);//0705rewrite initial value
    ...
    write_cmos_sensor(0x3815, 0x01);//0705rewrite initial value
	write_cmos_sensor(0x3816, 0x01);
    write_cmos_sensor(0x3817, 0x01);//0705rewrite initial value
	...
	write_cmos_sensor(0x4050, 0x04);
	write_cmos_sensor(0x4051, 0x0b);
	write_cmos_sensor(0x4837, 0x0f);//0e->0f modify mipi clck
	write_cmos_sensor(0x4902, 0x01);
...
static void slim_video_setting(void)
{
	...
	write_cmos_sensor(0x4837, 0x3a);//modify mipi clck
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		imgsensor.i2c_write_id = 0x18;
		if(s4AF_ReadReg_I2C(imgsensor.i2c_write_id))
		{
			#ifdef CONFIG_HUAWEI_HW_I2C_DCT
			set_hw_dev_flag(DEV_I2C_AF);
			#endif
		}
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		#ifdef CONFIG_HUAWEI_HW_I2C_DCT
		...
		#endif
		app_info_set("camera_main","OV13855-TRULY");
                return ERROR_NONE;
            }
