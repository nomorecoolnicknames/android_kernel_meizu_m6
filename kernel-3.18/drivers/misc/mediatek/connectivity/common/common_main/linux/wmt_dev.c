...
INT32 wmt_dev_read_file(PUINT8 pName, const PPUINT8 ppBufPtr, INT32 offset, INT32 padSzBuf)
{
	...
	*ppBufPtr = NULL;
	...
	/*file_len = fd->f_path.dentry->d_inode->i_size;*/
        err = vfs_getattr(&fd->f_path, &stat);
        if (err)
        {
            return -1;
        }
	...
	/*do {
        ...
        if(vfs_llseek(fd, offset, SEEK_SET) != offset)
        {
            WMT_ERR_FUNC("failed to seek!!\n");
	    goto read_file_done;
        }
	...
	while (pos < stat.size) {
                offset1 = pos;
                pos = vfs_read(fd, (void __user *)(pBuf + padSzBuf) + pos, stat.size - pos, &offset1);
		if (bytes < 0) {
			iRet = -1;
			goto read_file_done;
		}
	...
	*ppBufPtr = pBuf;
        ...
        if (read_len != file_len)
        {
	    WMT_ERR_FUNC("read abnormal: read_len(%d), file_len(%d)\n", read_len, file_len);
        }
read_file_done:
	if (iRet) {
		if (pBuf)
