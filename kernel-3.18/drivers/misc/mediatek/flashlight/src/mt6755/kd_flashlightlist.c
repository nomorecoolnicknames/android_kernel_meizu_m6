...
struct wake_lock flashlight_control_lock;
static int wakelockcount;
...
int flashlight_gpio_init(struct platform_device *pdev)
{
	...
	flashlight_flash_high = pinctrl_lookup_state(flashlight_pinctrl, "flashlight_high");
	if (IS_ERR(flashlight_flash_high)) {
		ret = PTR_ERR(flashlight_flash_high);
		logI("%s : init err, flashlight_flash_high\n", __func__);
	}

	flashlight_flash_low = pinctrl_lookup_state(flashlight_pinctrl, "flashlight_low");
...
int flashlight_gpio_set(int pin , int state)
{
	...
	case FLASHLIGHT_PIN_TORCH:
        if (state == STATE_LOW && !IS_ERR(flashlight_torch_low)){
            ...
            logI("%s : set err, pin(%d) state(%d)\n", __func__, pin, state);
#ifdef CONFIG_HUAWEI_DSM
            camera_report_flash_dsm_err(LED_VOUT_ERR);
#endif
            }
		break;
...
static void Lbat_protection_powerlimit_flash(LOW_BATTERY_LEVEL level)
{
	if (level == LOW_BATTERY_LEVEL_0) {
		...
		closeFlash();
#ifdef CONFIG_HUAWEI_DSM
        //camera_report_flash_dsm_err(LED_POWER_SAVING);
#endif
...
extern int rear_flashlight_operate(int duty,int open);
...
{
   logI("RunningTest rear flashlight,mode=%d \n",flashInfo.mode);
   seq_printf(m, "%d\n",flashInfo.mode);
   return 0;
};
                         ...
                         loff_t *data)
{
	...
	if (sscanf(regBuf, "%x %x",  &flashInfo.flash_id, &flashInfo.mode) == 2) {
		logI("[%s] flash_id= %d,mode = %d.", __FUNCTION__,flashInfo.flash_id, flashInfo.mode);
		if(flashInfo.flash_id & 0x001){	//rear first flash light
			if(flashInfo.mode == 0x2){//torch mode
				rear_flashlight_operate(0,1);
			}else if(flashInfo.mode == 0x1){//flash mode
				rear_flashlight_operate(1,1);
			}else{
				rear_flashlight_operate(0,0);
			}
		}else if(flashInfo.flash_id & 0x010){//rear sencond flash light
			if(flashInfo.mode){
				rear_flashlight_operate(0,1);
			}else{
				rear_flashlight_operate(0,0);
			}
		}else if(flashInfo.flash_id & 0x100){//front first flash light
			logI("[%s] unsupport Front flashlight.", __FUNCTION__);
		}else {
			logI("[%s] unsupport flashlight id.", __FUNCTION__);
		}
	}else if (sscanf(regBuf, "%x", &flashInfo.flash_id) == 1) {
		logI("[%s] unsupport flashlight cmd.", __FUNCTION__);
    }
...
/*wangjijin delet flashlight code*/
/*
