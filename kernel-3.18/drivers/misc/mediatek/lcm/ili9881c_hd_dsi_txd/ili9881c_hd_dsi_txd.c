/* forge 2026-07-25: ili9881c_hd_dsi_txd = ili9881p_hd_dsi_txd board/power path (TPS65132 bias,
 * M6 GPIOs) + ILI9881C panel init table & DSI timings from the in-tree ILITEK
 * reference. Target: M6 unit whose atag reports lcmname "ili9881c_hd_dsi_txd".
 */
/*
...
/* Stock Flyme LK/kernel ili9881c_hd_dsi_txd init table. */
...
#ifndef CONFIG_FPGA_EARLY_PORTING
#if (LCM_DSI_CMD_MODE)
	params->dsi.PLL_CLOCK = 230;	/* M6: 230->240 for 60Hz hardware scanout (was 57.65) */
#else
	params->dsi.PLL_CLOCK = 230;	/* M6: 230->240 for 60Hz hardware scanout (was 57.65) */
#endif
...
static void lcm_init(void)
{
	...
	if (ret < 0)
		LCM_LOGI("ili9881c_hd_dsi_txd----tps6132----cmd=%0x--i2c write error----\n", cmd);
	else
		LCM_LOGI("ili9881c_hd_dsi_txd----tps6132----cmd=%0x--i2c write success----\n", cmd);
	...
	if (lcm_dsi_mode == CMD_MODE) {
		...
		LCM_LOGI("M6 LCM init seq=%u push init table end mode=%d\n",
			seq, lcm_dsi_mode);
		LCM_LOGI("ili9881c_hd_dsi_txd----tps6132----lcm mode = vdo mode :%d----\n", lcm_dsi_mode);
	}
...
static unsigned int lcm_compare_id(void)
{
	...
	LCM_LOGI("%s,ili9881c_hd_dsi_txd_f2_id=0x%08x\n", __func__, id);
...
void lcm_m6_diag_page5_2a_probe(unsigned int value, unsigned int hold_ms)
{
	...
	LCM_LOGI("M6 LCM page5_2a_probe begin write=%02x hold_ms=%u\n",
		write_value, bounded);
	lcm_m6_diag_select_stock_page(5);
	memset(read_buf, 0xA5, sizeof(read_buf));
	read_count = read_reg_v2(0x2A, read_buf, 1);
	LCM_LOGI("M6 LCM page5_2a_probe before read=%02x %02x %02x %02x read_count=%u\n",
		read_buf[0], read_buf[1], read_buf[2], read_buf[3],
		read_count);

	dsi_set_cmdq_V2(0x2A, 1, &write_value, 1);
	MDELAY(2);
	memset(read_buf, 0xA5, sizeof(read_buf));
	read_count = read_reg_v2(0x2A, read_buf, 1);
	LCM_LOGI("M6 LCM page5_2a_probe after_write write=%02x read=%02x %02x %02x %02x read_count=%u\n",
	...
	memset(read_buf, 0xA5, sizeof(read_buf));
	read_count = read_reg_v2(0x2A, read_buf, 1);
	LCM_LOGI("M6 LCM page5_2a_probe hold_end write=%02x read=%02x %02x %02x %02x read_count=%u\n",
		write_value, read_buf[0], read_buf[1], read_buf[2],
		read_buf[3], read_count);
	lcm_m6_diag_select_stock_page(0);
	LCM_LOGI("M6 LCM page5_2a_probe end write=%02x reset_page=0\n",
		write_value);
}
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	if (log_this) {
		...
		LCM_LOGI("%s,ili9881c_hd_dsi_txd backlight: level = %d\n",
			__func__, level);
...
LCM_DRIVER ili9881c_hd_dsi_txd_lcm_drv = {
	.name = "ili9881c_hd_dsi_txd",
