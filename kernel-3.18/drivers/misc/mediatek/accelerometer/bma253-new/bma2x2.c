...
static int BMA2x2_CheckDeviceID(struct i2c_client *client)
{
	...
	//if(data==BMA222E_CHIPID||data==BMA250E_TYPE||data==BMA255_CHIPID||data==BMA280_CHIPID)
	if(CHIP_TYPE!=UNKNOWN_TYPE)
	{
...
static int bma2x2_app_set_info(struct i2c_client *client)
{
	int res = 0;
	int err=0;
	res = BMA2x2_CheckDeviceID(client);
	if (res != BMA2x2_SUCCESS)
	    return res;
	err = app_info_set("G-Sensor", "BMA253");
	if(err < 0){
		GSE_LOG("bma253 failed to add app_info\n");
	}
#ifdef CONFIG_HUAWEI_HW_I2C_DCT
...
//tuwenzan@wind-mobi.com add at 20161128 end
#ifdef CONFIG_HUAWEI_DSM
static ssize_t bma253_show_test_dsm(struct device_driver *ddri, char *buf)
{
...
*/
static ssize_t bma253_store_test_dsm(struct device_driver *ddri, const char *buf, size_t tCount)
{
    int mode;
    int ret=0;
...
#endif
#ifdef CONFIG_HUAWEI_DSM
static DRIVER_ATTR(dsm_excep, 0660, bma253_show_test_dsm, bma253_store_test_dsm);
#endif
/*----------------------------------------------------------------------------*/
	 ...
	 const struct i2c_device_id *id)
{
    ...
    bma2x2_app_set_info(new_client);
#ifdef CONFIG_MOTO_AOD_BASE_ON_AP_SENSORS
