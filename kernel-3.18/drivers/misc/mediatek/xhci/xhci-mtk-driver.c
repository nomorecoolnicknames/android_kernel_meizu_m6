...
static struct delayed_work check_adc_delaywork;
int otg_adc_value[5] = {0};
#endif
u32 xhci_debug_level = K_ALET | K_CRIT | K_ERR | K_WARNIN;
int mtk_iddig_debounce = 350;
...
#define GPIO_BOARD_ID (GPIO51 | 0x80000000)
int adc_threshold_max=0;
int adc_threshold_min=0;
#define ADC_VOLTAGE_LIMIT 	  150*1000   //mV
#define ADC_VOLTAGE_MAX         1250*1000 //mv
static int otg_isr_contral(void)
{
	int ret = 0;
	int id_volt = 0;
	int inserted = 0; //0 normal 1 bad
	int rc = 0;
	...
	printk("GPIO_BOARD_ID value is %d\n",mt_get_gpio_in(GPIO_BOARD_ID));
	if(rc == 0)//V4
	{
	adc_threshold_max = 1200*1000;//1200mv
	adc_threshold_min=0;
	}
	else
	{//vn1
	adc_threshold_max = 150*1000;//150mv
	adc_threshold_min=0;
	}
	...
	if (ret != 0)
		...
		if ((id_volt >= adc_threshold_min) && (id_volt <=adc_threshold_max)){
		printk("[otg_isr_contral]-otg plug in normal");
		inserted=0;
		}else{
		printk("[otg_isr_contral]-otg plug in bad");
		inserted=1;
		}
		return inserted;
}
#ifdef CONFIG_ATO_TEST
void check_adc_func(struct work_struct *work)
{
	int count = 1;
	while(count < 5)
	{
		IMM_GetOneChannelValue_Cali(14, &otg_adc_value[count]);
		if (((otg_adc_value[count] > ADC_VOLTAGE_LIMIT) && (otg_adc_value[count] <= ADC_VOLTAGE_MAX)))
			count++;
		else
			break;
		msleep(5000);
	...
	if(count >=5)
		disable_irq_wake(mtk_idpin_irqnum);
	else
...
void mtk_xhci_mode_switch(struct work_struct *work)
{
	...
	if(strstr(saved_command_line,recovery_mode_str)==NULL){
		printk("boot mode is normal_mode_str\n");
		normal_boot_mode = 1;
		}else{
		printk("boot mode is recovery_mode_str\n");
		normal_boot_mode = 0;
		}
	if((!(otg_isr_contral())&&(host_status==0x31))||(normal_boot_mode==0))
		printk("inserted success\n");
        else{
		otg_online = 0;
		irq_set_irq_type(mtk_idpin_irqnum, IRQF_TRIGGER_HIGH);
		enable_irq(mtk_idpin_irqnum);
...
static int mtk_xhci_eint_iddig_irq_en(void)
{
	...
	if (!otg_iddig_isr_enable) {
		...
		ret = IMM_GetOneChannelValue_Cali(14, &id_volt);
		if (ret != 0)
			printk("[start-otg_adc]id_volt read fail\n");
		else
		{
#ifdef CONFIG_ATO_TEST
			otg_adc_value[0] = id_volt;
#endif
			printk("[start-otg_adc]id_volt = %d\n", id_volt);
		}
		if( (((id_volt > ADC_VOLTAGE_LIMIT) && (id_volt <= ADC_VOLTAGE_MAX))||(pmic_chrdet_status() == KAL_TRUE))&&(rc==1))
			{
			printk("[start-otg_adc]mtk_xhci_eint_iddig_irq_en exec failed\n");
			#ifdef CONFIG_ATO_TEST
			schedule_delayed_work(&check_adc_delaywork, 0);
			#endif
			}else{
			printk("[start-otg_adc]mtk_xhci_eint_iddig_irq_en exec sucess\n");
			enable_irq_wake(mtk_idpin_irqnum);
...
int mtk_xhci_eint_iddig_init(void)
{
	...
	#ifdef CONFIG_ATO_TEST
	INIT_DELAYED_WORK(&check_adc_delaywork, check_adc_func);
	#endif
#ifdef CONFIG_MTK_OTG_PMIC_BOOST_5V
#ifdef CONFIG_MTK_OTG_OC_DETECTOR
...
void mtk_xhci_eint_iddig_deinit(void)
{
	...
	#ifdef CONFIG_ATO_TEST
	cancel_delayed_work(&check_adc_delaywork);
	#endif
#ifdef CONFIG_MTK_OTG_PMIC_BOOST_5V
#ifdef CONFIG_MTK_OTG_OC_DETECTOR
	cancel_delayed_work(&mtk_xhci_oc_delaywork);
