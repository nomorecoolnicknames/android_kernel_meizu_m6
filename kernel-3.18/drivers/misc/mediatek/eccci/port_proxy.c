...
int port_proxy_send_msg_to_user(struct port_proxy *proxy_p, CCCI_CH ch, CCCI_MD_MSG msg, u32 resv)
{
	...
	if (skb == NULL) {
		CCCI_ERROR_LOG(proxy_p->md_id, TAG, "ccci_alloc_skb fail for port_proxy_send_msg_to_user\n");
		return -1;
	}
...
int modem_dtr_set(int on, int low_latency)
{
	...
	if (proxy_p == NULL) {
		CCCI_ERROR_LOG(MD_SYS3, CHAR, "proxy_p NULL for modem_dtr_set\n");
		return -1;
	}
...
int ccci_c2k_rawbulk_intercept(int ch_id, unsigned int interception)
{
	...
	if (proxy_p == NULL) {
		ret = -ENODEV;
		CCCI_ERROR_LOG(MD_SYS3, CHAR, "Proxy_p NULL\n");
		return ret;
	}
...
int ccci_c2k_buffer_push(int ch_id, void *buf, int count)
{
	...
	if (proxy_p == NULL) {
		ret = -ENODEV;
		CCCI_ERROR_LOG(MD_SYS3, CHAR, "proxy_p NULL at ccci_c2k_buffer_push\n");
		return ret;
	}
