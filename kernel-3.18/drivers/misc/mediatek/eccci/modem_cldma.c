...
static int cldma_rx_ring_init(struct ccci_modem *md, struct cldma_ring *ring)
{
	int i;
	...
	if (ring->type == RING_GPD) {
		for (i = 0; i < ring->length; i++) {
			...
			if (item == NULL) {
				CCCI_ERROR_LOG(md->index, TAG, "cldma_rx_ring_init kzalloc fail\n");
				return -1;
			}
			item->gpd = dma_pool_alloc(md_ctrl->gpd_dmapool, GFP_KERNEL, &item->gpd_addr);
			if (item->gpd == NULL) {
				CCCI_ERROR_LOG(md->index, TAG, "cldma_rx_ring_init dma_pool_alloc fail\n");
				return -1;
			}
			item->skb = ccci_alloc_skb(ring->pkt_size, 1, 1);
			if (item->skb == NULL) {
				CCCI_ERROR_LOG(md->index, TAG, "cldma_rx_ring_init ccci_alloc_skb fail\n");
				return -1;
			}
...
static int cldma_tx_ring_init(struct ccci_modem *md, struct cldma_ring *ring)
{
	int i, j;
	...
	if (ring->type == RING_GPD) {
		for (i = 0; i < ring->length; i++) {
			item = kzalloc(sizeof(struct cldma_request), GFP_KERNEL);
			if (item == NULL) {
				CCCI_ERROR_LOG(md->index, TAG, "cldma_tx_ring_init kzalloc fail\n");
				return -1;
			}
			item->gpd = dma_pool_alloc(md_ctrl->gpd_dmapool, GFP_KERNEL, &item->gpd_addr);
			if (item->gpd == NULL) {
				CCCI_ERROR_LOG(md->index, TAG, "cldma_tx_ring_init dma_pool_alloc fail\n");
				return -1;
			}
	...
	if (ring->type == RING_GPD_BD) {
		for (i = 0; i < ring->length; i++) {
			item = kzalloc(sizeof(struct cldma_request), GFP_KERNEL);
			if (item == NULL) {
				CCCI_ERROR_LOG(md->index, TAG, "cldma_tx_ring_init kzalloc fail(BD)\n");
				return -1;
			}
			item->gpd = dma_pool_alloc(md_ctrl->gpd_dmapool, GFP_KERNEL, &item->gpd_addr);
			if (item->gpd == NULL) {
				CCCI_ERROR_LOG(md->index, TAG, "cldma_tx_ring_init dma_pool_alloc fail(BD)\n");
				return -1;
			}
			tgpd = (struct cldma_tgpd *)item->gpd;
			memset(tgpd, 0, sizeof(struct cldma_tgpd));
			...
			for (j = 0; j < MAX_BD_NUM + 1; j++) {	/* extra 1 BD for skb head */
				...
				if (bd_item == NULL) {
					CCCI_ERROR_LOG(md->index, TAG, "cldma_tx_ring_init kzalloc bd_item fail\n");
					return -1;
				}
				bd_item->gpd = dma_pool_alloc(md_ctrl->gpd_dmapool, GFP_KERNEL, &bd_item->gpd_addr);
				if (bd_item->gpd == NULL) {
					CCCI_ERROR_LOG(md->index, TAG,
							"cldma_tx_ring_init dma_pool_alloc bd_item fail\n");
					return -1;
				}
...
static int md_cd_start(struct ccci_modem *md)
{
	...
	if (md->config.setting & MD_SETTING_FIRST_BOOT) {
		if (md_cd_late_init(md) < 0) {
			CCCI_BOOTUP_LOG(md->index, TAG, "md_cd_late_init fail fro md_cd_start\n");
			return -1;
		}
...
static int md_cd_late_init(struct ccci_modem *md)
{
...
#endif
		ret = cldma_tx_ring_init(md, &md_ctrl->net_tx_ring[i]);
		if (ret < 0)
			return -1;
	...
	for (i = 0; i < NET_RXQ_NUM; i++) {
		...
		ret = cldma_rx_ring_init(md, &md_ctrl->net_rx_ring[i]);
		if (ret < 0)
			return -1;
...
#endif
		ret = cldma_tx_ring_init(md, &md_ctrl->normal_tx_ring[i]);
		if (ret < 0)
			return -1;
	...
	for (i = 0; i < NORMAL_RXQ_NUM; i++) {
		...
		ret = cldma_rx_ring_init(md, &md_ctrl->normal_rx_ring[i]);
		if (ret < 0)
			return -1;
