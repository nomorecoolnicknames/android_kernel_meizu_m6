...
{
    if (mt_mt65xx_blink_set(led_cdev, delay_on, delay_off, raise_duration, fall_duration))
		return -1;
	else
		return 0;
}
...
#endif
int g_ata_flag = 1;
static ssize_t show_ata_flag(struct device *dev, struct device_attribute *attr,
             char *buf)
{
	LEDS_DRV_DEBUG("g_ata_flag = :%d\n", g_ata_flag);
	return sprintf(buf, "%u\n", g_ata_flag);
}

static ssize_t store_ata_flag(struct device *dev, struct device_attribute *attr,
              const char *buf, size_t size)
{
	char *pvalue = NULL;
    unsigned int ata_flag = 0;

    if(buf!=NULL && size!=0)
    {
        LEDS_DRV_DEBUG("buf is %s.\n",buf);
        ata_flag=simple_strtol(buf,&pvalue,10);
        if((ata_flag == 0)||(ata_flag == 1))
            g_ata_flag = ata_flag;
        else
            LEDS_DRV_DEBUG("eg:echo[0/1]>ata_flag.\n");
	}

	return size;
}

static DEVICE_ATTR(ata_flag, 0664, show_ata_flag, store_ata_flag);
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	int ret;/* rc; */
	struct cust_mt65xx_led *cust_led_list = mt_get_cust_led_list();
	ret = gpio_request(LCT_LED_PM_CTRL_PIN,NULL);
	if(ret){
		LEDS_DRV_DEBUG ("LCT_LED_PM_CTRL_PIN request failed");
	}
	ret = gpio_direction_output(LCT_LED_PM_CTRL_PIN,GPIO_DIR_OUT);
	if(ret){
		LEDS_DRV_DEBUG ("LCT_LED_PM_CTRL_PIN set direction failed");
	}
