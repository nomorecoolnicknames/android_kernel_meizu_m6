...
#define LTR578_DEV_NAME   "LTR_559ALS"
...
static int ltr578_init_device(void);
...
static ssize_t ltr578_dynamic_calibrate(void)
{
	/* int ret=0; */
	int i = 0;
	int data;
...
static ssize_t ltr578_show_status(struct device_driver *ddri, char *buf)
{
	...
	mutex_lock(&Ltr559_lock);
		...
		len += snprintf(buf + len, PAGE_SIZE - len, "CUST: %d, (%d %d)\n",
				ltr578_obj->hw.i2c_num, ltr578_obj->hw.power_id,
...
static ssize_t ltr578_store_status(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (1 == sscanf(buf, "%d ", &status1)) {
		mutex_lock(&Ltr559_lock);
		ret = ltr578_ps_enable(ps_gainrange);
		mutex_unlock(&Ltr559_lock);
...
static int ltr578_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		...
		if (err) {
			APS_ERR("driver_create_file (%s) = %d\n", ltr578_attr_list[idx]->attr.name,
				err);
			break;
		}
	}
...
static int ltr578_ps_enable(int gainrange)
{
	...
	APS_LOG("LTR578_PS setgain = %d!\n", setgain);

	error = ltr578_i2c_write_reg(LTR578_PS_CONTR, setgain);
	if (error < 0) {
		APS_LOG("ltr578_ps_enable() error1\n");
		return error;
	}
	/* wisky-lxh@20150108 */
	res = ltr578_init_device();
	if (res < 0) {
		APS_ERR("ltr578_init_devicet: %d\n", res);
		return res;
	}
	/* end-wisky-lxh */
	...
	data = ltr578_i2c_read_reg(LTR578_PS_CONTR);
	...
	if (0 == obj->hw.polling_mode_ps) {
		...
		databuf[0] = LTR578_INTERRUPT_PERSIST;
		databuf[1] = 0x20;
		res = i2c_master_send(client, databuf, 0x2);
		if (res <= 0) {
			goto EXIT_ERR;
...
static int ltr578_ps_disable(void)
{
	...
	error = ltr578_i2c_write_reg(LTR578_PS_CONTR, MODE_PS_StdBy);
	if (error < 0)
...
static int ltr578_als_enable(int gainrange)
{
	...
	switch (gainrange) {
	case ALS_RANGE_64K:
		error = ltr578_i2c_write_reg(LTR578_ALS_CONTR, MODE_ALS_ON_Range1);
		break;

	case ALS_RANGE_32K:
		error = ltr578_i2c_write_reg(LTR578_ALS_CONTR, MODE_ALS_ON_Range2);
		break;

	case ALS_RANGE_16K:
		error = ltr578_i2c_write_reg(LTR578_ALS_CONTR, MODE_ALS_ON_Range3);
		break;

	case ALS_RANGE_8K:
		error = ltr578_i2c_write_reg(LTR578_ALS_CONTR, MODE_ALS_ON_Range4);
		break;

	case ALS_RANGE_1300:
		error = ltr578_i2c_write_reg(LTR578_ALS_CONTR, MODE_ALS_ON_Range5);
		break;

	case ALS_RANGE_600:
		error = ltr578_i2c_write_reg(LTR578_ALS_CONTR, MODE_ALS_ON_Range6);
		break;
...
static int ltr578_als_disable(void)
{
	...
	error = ltr578_i2c_write_reg(LTR578_ALS_CONTR, MODE_ALS_StdBy);
	if (error < 0)
...
static int ltr578_als_read(int gainrange)
{
	...
	alsval_ch1_lo = ltr578_i2c_read_reg(LTR578_ALS_DATA_CH1_0);
	alsval_ch1_hi = ltr578_i2c_read_reg(LTR578_ALS_DATA_CH1_1);
	alsval_ch1 = (alsval_ch1_hi * 256) + alsval_ch1_lo;
	alsval_ch0_lo = ltr578_i2c_read_reg(LTR578_ALS_DATA_CH0_0);
	alsval_ch0_hi = ltr578_i2c_read_reg(LTR578_ALS_DATA_CH0_1);
	...
	if (luxdata_int < (als_level[current_tp][current_color_temp][3] - 50)) {
		if (als_gainrange != ALS_RANGE_600) {
			ltr578_i2c_write_reg(LTR578_ALS_CONTR, MODE_ALS_ON_Range6);
		...
		if (als_gainrange != ALS_RANGE_64K) {
			ltr578_i2c_write_reg(LTR578_ALS_CONTR, MODE_ALS_ON_Range1);
...
/* eint request */
	if (ltr578_obj->irq_node) {
#if 0
		of_property_read_u32_array(ltr578_obj->irq_node, "debounce", ints,
					   ARRAY_SIZE(ints));
		...
		if (request_irq
		    (ltr578_obj->irq, ltr578_eint_handler, IRQF_TRIGGER_LOW, "ALS-eint", NULL)) {
	...
	mt_eint_registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_TYPE, ltr578_eint_func, 0);
...
/*----------------------------------------------------------------------------*/
/*for interrupt work mode support -- by liaoxl.lenovo 12.08.2011*/
static int ltr578_check_and_clear_intr(struct i2c_client *client)
{
/* *** */
	int res, intp, intl;
	u8 buffer[2];
...
static int ltr578_init_device(void)
{
	...
	error = ltr578_i2c_write_reg(LTR578_PS_LED, 0x7F);
	if (error < 0) {
		APS_LOG("ltr578_ps_enable() error3...\n");
		return error;
	}

	error = ltr578_i2c_write_reg(LTR578_PS_N_PULSES, 0x4);	/* modify by hzb */
	if (error < 0) {
		APS_LOG("ltr578_ps_enable() error2\n");
		return error;
	}

	error = ltr578_i2c_write_reg(LTR578_ALS_MEAS_RATE, 0x1);
	if (error < 0) {
		APS_LOG("ltr578_ps_enable() error2\n");
		return error;
	}

	error = ltr578_i2c_write_reg(LTR578_PS_THRES_UP_0, ps_trigger_high & 0xff);
	if (error < 0) {
		APS_LOG("ltr578_ps_enable() error2\n");
		return error;
	}
	error = ltr578_i2c_write_reg(LTR578_PS_THRES_UP_1, (ps_trigger_high >> 8) & 0X07);
	if (error < 0) {
		APS_LOG("ltr578_ps_enable() error2\n");
		return error;
	}
	error = ltr578_i2c_write_reg(LTR578_PS_THRES_LOW_0, 0x0);
	if (error < 0) {
		APS_LOG("ltr578_ps_enable() error2\n");
		return error;
	}
	error = ltr578_i2c_write_reg(LTR578_PS_THRES_LOW_1, 0x0);
	if (error < 0) {
		APS_LOG("ltr578_ps_enable() error2\n");
		return error;
	}
...
static int ltr578_devinit(void)
{
	...
	/* end-wisky-lxh */
	res = ltr578_check_and_clear_intr(client);
	if (res < 0) {
...
{
	struct ltr578_priv *obj =
	    (struct ltr578_priv *)container_of(work, struct ltr578_priv, eint_work);
	int err;
	struct hwm_sensor_data sensor_data;
/* int temp_noise; */
/* u8 buffer[1]; */
/* u8 reg_value[1]; */
	...
	if (err < 0) {
		APS_ERR("ltr578_eint_work error check intrs: %d\n", err);
	} else {
		/* get raw data */
		...
		APS_DBG("ltr578_eint_work rawdata ps=%d als_ch0=%d!\n", obj->ps, obj->als);
		      ...
		      void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_ENABLE:
		if ((buff_in == NULL) || (size_in < sizeof(int))) {
			...
			value = *(int *)buff_in;
			if (value) {
				err = ltr578_ps_enable(ps_gainrange);
				if (err < 0) {
		       ...
		       void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(struct hwm_sensor_data))) {
			...
			sensor_data->values[0] = ltr578_get_als_value(obj, obj->als);	/* wisky-lxh@20150206 */
#else
...
static int ltr578_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	strlcpy(info->type, LTR578_DEV_NAME, sizeof(info->type));
	return 0;
}
...
static int ps_enable_nodata(int en)
{
	...
	if (en) {
		res = ltr578_ps_enable(ps_gainrange);
		if (res < 0)
			goto err_out;
		set_bit(CMC_BIT_PS, &ltr578_obj->enable);
	} else {
...
static int ltr578_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = get_alsps_dts_func_ltr578(client->dev.of_node, &obj->hw);
	if (err < 0) {
...
#ifdef USE_HWMSEN
	obj_ps.self = ltr578_obj;
	/*for interrupt work mode support -- by liaoxl.lenovo 12.08.2011 */
	if (1 == obj->hw.polling_mode_ps)
		obj_ps.polling = 1;
	else
		obj_ps.polling = 0;
	obj_ps.sensor_operate = ltr578_ps_operate;
	...
	obj_als.self = ltr578_obj;
	obj_als.polling = 1;
	obj_als.sensor_operate = ltr578_als_operate;
	...
	obj->early_drv.level = EARLY_SUSPEND_LEVEL_DISABLE_FB - 1,
	    ...
	    obj->early_drv.resume = ltr578_late_resume, register_early_suspend(&obj->early_drv);
#endif
	APS_LOG("%s: OK\n", __func__);
	ltr578_init_flag = 1;
	return 0;
