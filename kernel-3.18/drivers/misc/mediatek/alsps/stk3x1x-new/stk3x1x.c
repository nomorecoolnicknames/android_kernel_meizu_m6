...
#define APS_LOG(fmt, args...)    pr_debugAPS_TAG fmt, ##args)
...
*******************************************************************************/
#ifdef STK_TUNE0
    #define STK_MAX_MIN_DIFF    110
    #define STK_LT_N_CT    54
    #define STK_HT_N_CT    97
...
struct alsps_hw alsps_cust2;
static struct alsps_hw *hw = &alsps_cust2;
struct platform_device *alspsPltFmDev2;
/*----------------------------------------------------------------------------*/
...
struct stk3x1x_priv {
    ...
    int als_comperator_D;
    int als_comperator_A;
    u16 als_last;
#ifdef STK_GES
...
#define STK3X1X_PRESISE_VALUE 400
#define STK_DEFAULT_VALUE 100
BOOL stk3x1x_is_calibrate = FALSE;
static int stk_final_lux_val;
static int stk_ration_value = 1;
...
void stk_als_ir_get_corr(struct stk3x1x_priv *obj, u32 als_data)//
{
    if(obj->ir_code)
    {
        obj->als_correct_factor = 1934;//cWF 1000*/
        if(als_data < STK_IRC_MAX_ALS_CODE && als_data > STK_IRC_MIN_ALS_CODE &&
            obj->ir_code > STK_IRC_MIN_IR_CODE)
        {
            obj->als_comperator_D = als_data * 8;//310
            obj->als_comperator_A = als_data * 41;
            if(obj->ir_code > obj->als_comperator_A)/*A*/
                obj->als_correct_factor = 603;
            else if((obj->ir_code > obj->als_comperator_D))/*D*/
                obj->als_correct_factor = 803;
        }
    }
    APS_LOG("%s: als_correct_factor=%d\n",__func__,obj->als_correct_factor);
    return;
}
/*----------------------------------------------------------------------------*/
#ifdef STK_IRS
...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
    ...
    if(obj->p_wv_r_bd_with_co & 0b010)
    {
        if(als_data < STK_ALS_THRESHOLD && obj->als_code_last > 10000)
        {
            ...
            APS_LOG("%s: obj->ir_code=%d\n",  __func__, obj->ir_code);
#endif
...
#endif
    als_raw_data = als_data;
    if(1 == stk_ration_value)
        *data = (u16)(als_data * stk_ration_value);
    else
        *data = (u16)(als_data * stk_ration_value / STK_DEFAULT_VALUE);

    stk_final_lux_val = *data;

    APS_DBG("5:als_data = %d\n",als_data);
...
static void stk3x1x_power(struct alsps_hw *hw, unsigned int on)
{
    ...
    if(on == 1){
        ...
        if (ret)
            APS_ERR("regulator_enable() failed!\n");
    }else{
        ret = regulator_disable(stk3x1x_regulator);       /*enable regulator*/
         if (ret)
            APS_ERR("regulator_disable() failed!\n");
    }
#endif
}
/*----------------------------------------------------------------------------*/
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
    ...
    if(enable == 0)
    {
        err = stk3x1x_read_flag(obj->client, &int_flag);
        APS_LOG("%s,frist read int_flag = 0x%x,res = %d\n",__func__,int_flag,err);
        if(err)
            return err;
        int_flag &= 0xEF;
        if((err = stk3x1x_write_flag(client, int_flag)))
        {
        ...
        err = stk3x1x_read_flag(obj->client, &int_flag);
        APS_LOG("%s,after write int_flag = 0x%x,res = %d\n",__func__,int_flag,err);
    }
    cur = old;
    cur &= (~(0x45));
    if(enable)
...
static int stk_tune1_ct_det(struct stk3x1x_priv *obj, uint32_t ps)
{
    ...
    if(err == 0xFFFF)
    {
        ...
        {
            APS_ERR("%s: invalid_ps_cnt > limit\n", __func__);
            return 3;
        }
        return 0;
    }
    ...
    if(ct >= 0)
    {
        ...
        else
        {
            ...
            APS_ERR("%s: ct>STK_PS_CT_LIMIT,keep HT/LT\n", __func__);
            return 2;
        }
    }
    return 0;
...
static int stk_tune1_ps_int_handle(struct stk3x1x_priv *obj)
{
    ...
    APS_ERR("%s:handle_state=%d,ps=%d,als=%d,nf_flag=%d,ps_distance_last=%d\n",
    ...
    case STK_PSINT_NORM:
        ...
        APS_ERR("%s:(N) ps input event=%d, ps code=%d\n",__func__, nf_state, obj->ps);
        if(nf_state)
        {
            stk3x1x_write_ps_high_thd(obj->client, atomic_read(&obj->ps_high_thd_val));
            stk3x1x_write_ps_low_thd(obj->client, 0);
            obj->stk_int_handle_state = STK_PSINT_NORM;
            APS_ERR("%s:(N) HT=%d,LT=0\n", __func__, ps_thd_h);
        }
        else
        {
    ...
    case STK_PSINT_FIN_DET:
        ...
        {
            stk_ps_report(obj, nf_state);
            APS_ERR("%s:(F) ps input event=%d, ps=%d\n",__func__, nf_state, obj->ps);
            stk3x1x_write_ps_high_thd(obj->client,  atomic_read(&obj->ps_high_thd_val));
            stk3x1x_write_ps_low_thd(obj->client, 0);
            obj->stk_int_handle_state = STK_PSINT_NORM;
            APS_ERR("%s:(F) HT=%d,LT=0\n", __func__, ps_thd_h);
        }
        break;
    case STK_PSINT_CT_DET:
        ...
        if(ret > 0)
        {
            ...
            stk3x1x_write_ps_low_thd(obj->client, 0);
            obj->stk_int_handle_state = STK_PSINT_NORM;
            APS_ERR("%s:(C) HT=%d,LT=0\n", __func__, ps_thd_h);
        }
        if(!nf_state)
        {
            stk_ps_report(obj, nf_state);
            APS_ERR("%s:(C) ps input event=%d, ps=%d\n",__func__, nf_state, obj->ps);
            stk3x1x_write_ps_high_thd(obj->client, STK_FIN_THD);
            stk3x1x_write_ps_low_thd(obj->client, atomic_read(&obj->ps_low_thd_val));
            obj->stk_int_handle_state = STK_PSINT_FIN_DET;
            APS_ERR("%s:(C) HT=%d,LT=%d\n", __func__, STK_FIN_THD, ps_thd_l);
        }
        break;
    default:
        APS_ERR(KERN_ERR "%s: invalid stk_int_handle_state %d\n", __func__, obj->stk_int_handle_state);
    }
    ...
    else if(ret == 0x10 && nf_state == 1)
    {
        ...
        APS_ERR("%s: HT=0,LT=0xffff\n", __func__);
        stk_tune1_reset_para();
        APS_ERR("%s: reset ps_get_ct\n", __func__);
    }
    return 0;
}
#else
...
int stk3x1x_setup_eint(struct i2c_client *client)
{
    ...
    alspsPltFmDev2 = get_alsps_platformdev();
/* gpio setting */
    pinctrl = devm_pinctrl_get(&alspsPltFmDev2->dev);
    if (IS_ERR(pinctrl)) {
        ret = PTR_ERR(pinctrl);
...
static int stk3x1x_init_client(struct i2c_client *client)
{
    ...
    if(obj->first_boot == true)
    {
    if(0== stk3x1x_read_id(client))
    {
        err = app_info_set("ALSPS-Sensor", "STK3311");
        if(err<0){
            APS_ERR("STK3311 failed to add app_info\n");
        }
#ifdef CONFIG_HUAWEI_HW_I2C_DCT
...
static ssize_t stk3x1x_store_send(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    dat = (u8)cmd;
    APS_LOG("send(%02X, %02X) = %d\n", addr, cmd,stk3x1x_master_send(stk3x1x_obj->client, (u16)addr, &dat, sizeof(dat)));
    return count;
}
/*----------------------------------------------------------------------------*/
...
static ssize_t stk3x1x_show_light_value(struct device_driver *ddri,char *buf)
{
    ssize_t len = 0;
...
static ssize_t stk3x1x_show_calibrate_cmd(struct device_driver *ddri,char *buf)
{
    ssize_t len = 0;
...
static ssize_t stk3x1x_store_calibrate_cmd(struct device_driver *ddri, const char *buf, size_t count)
{
    int cmd;

    if(!stk3x1x_obj)
    {
        APS_ERR("stk3x1x is null!\n");
        return 0;
    }
    else if(1 == sscanf(buf,"%d",&cmd))
    {
    ...
    switch(cmd)
    {
        case 0:
        {
            stk3x1x_is_calibrate = FALSE;
            APS_DBG("stk3x1x_is_calibrate cmd is %d\n",stk3x1x_is_calibrate);
            break;
        }
        case 1:
        {
            stk3x1x_is_calibrate = TRUE;
            break;
        }
        default:
            APS_DBG("unknown cmd\n");
...
static ssize_t stk3x1x_show_calibrate_ration(struct device_driver *ddri,char *buf)
{
    ssize_t len = 0;
    len += snprintf(buf,PAGE_SIZE-len,"stk_ration_value = %d\n",stk_ration_value);
    return len;
}
static ssize_t stk3x1x_store_calibrate_ration(struct device_driver *ddri, const char *buf, size_t count)
{
    int value = 0;
    ...
    {
        APS_DBG("before store calibrate_ration the    stk_ration_value is %d\n",stk_ration_value);
        stk_ration_value = value;
        APS_DBG("after store calibrate_ration the  stk_ration_value is %d\n",stk_ration_value);
    }
    else
    {
        APS_DBG("invalid content: %s, length = %zu\n", buf, count);
...
#endif
/*chip info*/
static DRIVER_ATTR(chipinfo,0644,stk3x1x_show_chipinfo,NULL);
/*display various value*/
static DRIVER_ATTR(als_cal_show,0644,stk3x1x_show_light_value,NULL);
/*start cail*/
static DRIVER_ATTR(als_cal_start,0644,stk3x1x_show_calibrate_cmd,stk3x1x_store_calibrate_cmd);
/*show and store ration value*/
static DRIVER_ATTR(als_cal_value,0644,stk3x1x_show_calibrate_ration,stk3x1x_store_calibrate_ration);
/*----------------------------------------------------------------------------*/
...
//    obj->hw = get_cust_alsps_hw();
    ...
    atomic_set(&obj->state_val, 0);
    atomic_set(&obj->psctrl_val, 0x30);
    atomic_set(&obj->alsctrl_val, 0x38);
    obj->ledctrl_val = 0xFF;
    obj->wait_val = 0x7;//0xf
    obj->int_val = 0;
    obj->first_boot = true;
...
static int __init stk3x1x_init(void)
{
    ...
    if(!stk3x1x_regulator)
        APS_ERR("stk3x1x_regulator is NULL\n");
    ret = regulator_set_voltage(stk3x1x_regulator, 2800000, 2800000); /*set 2.8v*/
    if (ret){
        APS_ERR("regulator_set_voltage() failed!\n");
    }
    msleep(3);
#endif
    //APS_ERR("stk3x1x_init Out");
    return 0;
}
/*----------------------------------------------------------------------------*/
