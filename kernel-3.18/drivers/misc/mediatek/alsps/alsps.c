...
static bool is_doing_als_stub = false;
static int als_stub_value = 0;

struct timer_list	    als_resume_timer;
struct work_struct	    als_resume_work;
unsigned int  als_resume_zero_count = 0;
unsigned int  als_resume_report_count = 0;
#define  ALS_RESUME_RATE  80
#define  ALS_RESUME_RATE1  55
#define  ALS_RESUME_ZERO_COUNT 10
#define ALS_RESUME_REPORT_COUNT 10
static int als_real_enable(int enable);

static void  als_resume_polling (unsigned long data)
{
	struct alsps_context *obj = (struct alsps_context *)data;

	if (obj != NULL){
		ALSPS_LOG("%s \n", __func__);
		schedule_work(&als_resume_work);
	}
}

static void als_resume_work_func(struct work_struct *work)
{
	...
	if ( cxt != NULL){
		mutex_lock(&cxt->alsps_op_mutex);
		err = cxt->als_data.get_data(&value, &status);

		if (err) {
			ALSPS_ERR("%s, get als data error \n", __func__);
		} else {
			if( value == 0 ) {
				als_resume_zero_count++;
				if(als_resume_zero_count <= 1 ){
					ALSPS_LOG("%s, first als_value is 0, not report,  only restart timer  \n", __func__);
					mod_timer(&als_resume_timer, jiffies +  ALS_RESUME_RATE1 / (1000 / HZ));
				}else if ( als_resume_zero_count  < ALS_RESUME_ZERO_COUNT) {
					ALSPS_LOG("%s,  als_value is 0,  zero_count=%d,  not first, so report and restart timer \n", __func__, als_resume_zero_count);
					als_data_report(cxt->idev, 1, status);
					mod_timer(&als_resume_timer, jiffies +  ALS_RESUME_RATE1 / (1000 / HZ));
				}else{
					ALSPS_LOG("%s,  als_value is 0  has been 10 timers, delete timer \n ", __func__);
					smp_mb();
					del_timer(&als_resume_timer);
					smp_mb();
				}
			}else{
				ALSPS_LOG("%s, als_data_report als_value=%d \n", __func__, value);
				als_data_report(cxt->idev, value, status);

				if( als_resume_report_count++ < ALS_RESUME_REPORT_COUNT ) {
					mod_timer(&als_resume_timer, jiffies +  ALS_RESUME_RATE1 / (1000 / HZ));
				}else{
					ALSPS_LOG("%s, delete als_resume_timer \n",  __func__);
		...
		mutex_unlock(&cxt->alsps_op_mutex);
	}
}

void als_resume_enable(void)
{
	struct alsps_context *cxt = NULL;
	...
	ALSPS_LOG("%s, start timer \n",  __func__ );

	mod_timer(&als_resume_timer, jiffies +  ALS_RESUME_RATE / (1000 / HZ));
...
static void als_work_func(struct work_struct *work)
{
	...
	err = cxt->als_data.get_data(&value, &status);
    if (true == is_doing_als_stub)
    {
        static int alsStubValueVibrate = 0;/*0 or 1 to pass report optimization*/
        value = als_stub_value;
        value += alsStubValueVibrate;
        alsStubValueVibrate = (alsStubValueVibrate + 1) % 2;
        err = 0;
        status = 2;/*medium accuracy*/
    }
	if (err) {
...
static struct alsps_context *alsps_context_alloc_object(void)
{
	...
	INIT_WORK(&als_resume_work, als_resume_work_func);
	init_timer(&als_resume_timer);
	als_resume_timer.function =  als_resume_polling;
	als_resume_timer.data	= (unsigned long)obj;
...
static int als_enable_data(int enable)
{
    ...
    if (1 == enable)
    {
        ALSPS_LOG("ALSPS enable data;%d;%d;%d;\n", cxt->is_als_polling_run, cxt->is_als_batch_enable, cxt->als_ctl.is_report_input_direct);
		cxt->is_als_active_data = true;
		cxt->is_als_first_data_after_enable = true;
		cxt->als_ctl.open_report_data(1);
        ...
        if (false == cxt->is_als_polling_run && cxt->is_als_batch_enable == false)
        {
            if (false == cxt->als_ctl.is_report_input_direct)
            {
                ...
                cxt->is_als_polling_run = true;  //move by hzb
                ALSPS_LOG("ALSPS polling timer start,delay is %d\n", atomic_read(&cxt->delay_als));
                mod_timer(&cxt->timer_als, jiffies + atomic_read(&cxt->delay_als) / (1000 / HZ));
            }
	...
	if (0 == enable) {
		...
		if (true == cxt->is_als_polling_run) {
			if (false == cxt->als_ctl.is_report_input_direct) {
				...
				smp_mb();/* for memory barrier */
                                del_timer(&cxt->timer_als);  //modify by hzb
                                smp_mb();/* for memory barrier */
                                //cancel_work_sync(&cxt->report_als);  //delete by hzb
	...
	if (1 == enable) {
		...
		if (false == cxt->is_ps_polling_run && cxt->is_ps_batch_enable == false) {
			if (false == cxt->ps_ctl.is_report_input_direct) {
                            ...
                            ALSPS_LOG("PS polling timer start,delay is %d\n", atomic_read(&cxt->delay_ps));
                            mod_timer(&cxt->timer_ps, jiffies + atomic_read(&cxt->delay_ps) / (1000 / HZ));
                            //-move by hzb
			}
		}
	...
	if (0 == enable) {
		...
		if (true == cxt->is_ps_polling_run) {
			if (false == cxt->ps_ctl.is_report_input_direct) {
				...
				smp_mb();/* for memory barrier*/
                                del_timer(&cxt->timer_ps);  //modify by hzb
                                smp_mb();/* for memory barrier*/
                                //cancel_work_sync(&cxt->report_ps);  //modify by hzb
				  ...
				  const char *buf, size_t count)
{
	...
	if (false == cxt->als_ctl.is_report_input_direct) {
		mdelay = (int)delay/1000/1000;
                mdelay = (mdelay >= 10) ? mdelay : 10; //add by hzb
				 ...
				 struct device_attribute *attr, char *buf)
{
    ...
    delay = atomic_read(&alsps_context_obj->delay_als);
    ALSPS_LOG("als delay is %d\n", delay);
	...
	if (cxt->als_ctl.is_support_batch) {
		if (!strncmp(buf, "1", 1)) {
			...
			if (true == cxt->is_als_polling_run) {
				cxt->is_als_polling_run = false;
                                del_timer(&cxt->timer_als);  //modify by hzb
                                //cancel_work_sync(&cxt->report_als);  //modify by hzb
				cxt->drv_data.als_data.values[0] = ALSPS_INVALID_VALUE;
...
static ssize_t als_show_stub(struct device* dev,
                             struct device_attribute* attr, char* buf)
{
    return snprintf(buf, PAGE_SIZE, "%d\n", als_stub_value);
}
static ssize_t als_store_stub(struct device* dev, struct device_attribute* attr,
                              const char* buf, size_t count)
{
    int ret = -1;

    is_doing_als_stub = true;

    ret = kstrtoint(buf, 16, &als_stub_value);
...
static ssize_t als_show_state(struct device* dev,
                              struct device_attribute* attr, char* buf)
{
    int als_state = 0;
    ...
    if (true == (alsps_context_obj->is_als_polling_run))
    {
        als_state = 1;
    }
    else
				  ...
				  const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "1", 1)){
		if ( esd_recovery_level  == 0 ) {
			als_resume_zero_count = 0;
			als_resume_report_count = 0;
			als_resume_enable();
		}
	}else if (!strncmp(buf, "0", 1))
		ps_enable_data(0);
				 ...
				 struct device_attribute *attr, char *buf)
{
    ...
    delay = atomic_read(&alsps_context_obj->delay_ps);
    ALSPS_LOG("ps delay is %d\n", delay);
...
static ssize_t ps_show_state(struct device* dev,
                             struct device_attribute* attr, char* buf)
{
    int ps_state = 0;
    ...
    if (true == (alsps_context_obj->is_ps_polling_run))
    {
        ps_state = 1;
    }
    else
...
DEVICE_ATTR(alsstub,        S_IWUSR | S_IRUGO, als_show_stub,  als_store_stub);
DEVICE_ATTR(alsstate,        S_IWUSR | S_IRUGO, als_show_state,  NULL);
DEVICE_ATTR(psstate,        S_IWUSR | S_IRUGO, ps_show_state,  NULL);
static struct attribute *alsps_attributes[] = {
    ...
    &dev_attr_alsstub.attr,
    &dev_attr_alsstate.attr,
    &dev_attr_psstate.attr,
	NULL
};
