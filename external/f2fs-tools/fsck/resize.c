...
static void migrate_nat_res_meta(struct f2fs_super_block *sb, struct f2fs_super_block *new_sb)
{
       block_t block_off, block_addr;
       unsigned int nr_nat_blks, seg_off, index;
...
static void migrate_ssa_res_meta(struct f2fs_sb_info *sbi, struct f2fs_super_block *new_sb,
					unsigned int offset)
{
       unsigned int segno, segnum;
       block_t block_addr;
                       ...
                       struct f2fs_super_block *new_sb, unsigned int offset, int reserved_meta)
{
       ...
       struct f2fs_super_block *sb = F2FS_RAW_SUPER(sbi);
       unsigned int free_segment_count, new_segment_count, j;
	...
	int ret, i;
       ...
       *((__le32 *)((unsigned char *)new_cp + CHECKSUM_OFFSET)) = cpu_to_le32(crc);
       ...
       for (j = 0; j < get_newsb(cp_payload); j++) {
...
old_version_userdata:
       if (reserved_meta) {
		...
		migrate_nat_res_meta(sb, new_sb);
		migrate_ssa_res_meta(sbi, new_sb, offset_seg);
