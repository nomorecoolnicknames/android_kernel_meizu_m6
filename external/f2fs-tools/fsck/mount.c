...
int validate_super_block(struct f2fs_sb_info *sbi, int block)
{
	...
	if (!sanity_check_raw_super(sbi->raw_super, offset)) {
		/* get kernel version */
		if (config.kd >= 0) {
			if (dev_read_version(config.version, 0, VERSION_LEN) < 0)
			{
		...
		if (cmp_kernel_version(config.sb_version, config.version)) {
			int ret;
...
void build_sit_area_bitmap(struct f2fs_sb_info *sbi)
{
	...
	ASSERT(fsck->sit_area_bitmap != NULL);
...
int find_next_free_block(struct f2fs_sb_info *sbi, u64 *to, int left, int type)
{
	...
	u64 offset;
	int not_enough = 0;
	u64 end_blkaddr = ((u64)le32_to_cpu(sb->segment_count_main) <<
			le32_to_cpu(sb->log_blocks_per_seg)) + (u64)le32_to_cpu(sb->main_blkaddr);
	...
	while (*to >= SM_I(sbi)->main_blkaddr && *to < end_blkaddr) {
		...
		if (se->valid_blocks == sbi->blocks_per_seg ||
			...
			*to = left ? START_BLOCK(sbi, segno) - 1:
						START_BLOCK(sbi, segno + 1); /*lint !e647*/
			continue;
		}
