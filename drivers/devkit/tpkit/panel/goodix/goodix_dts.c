	...
	struct goodix_ts_data *goodix_pdata)
{
	...
	//goodix_prase_delay_config_dts(np, goodix_pdata);
...
#if 0
	ret = of_property_read_u32(np, GTP_PRAM_PROJECTID_ADDR, &goodix_pdata->pram_projectid_addr);
	if (ret) {
		/* Modify@wangdongbo.wt/wwx378215_20161215_need_check */
		goodix_pdata->pram_projectid_addr = GTP_BOOT_PROJ_CODE_ADDR2;
		TS_LOG_ERR("%s:get pram_projectid_addr from dts failed ,use default FT8716 pram addr\n", __func__);
	}
#endif
	...
	ret = of_property_read_u32(np,  "need_wd_check_status", &dev_data->capacitance_test_config);
	if (ret) {
		TS_LOG_INFO("%s, get device need_wd_check_status failed, will use default value: 0 \n ", __func__);
