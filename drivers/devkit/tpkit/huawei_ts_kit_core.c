...
static int ts_charger_detect_cmd(enum ts_charger_state charger_state)
{
	...
	TS_LOG_INFO("xljadd charger detect\n");
	cmd = (struct ts_cmd_node *)kzalloc(sizeof(struct ts_cmd_node),GFP_KERNEL);
	if (!cmd) {
		TS_LOG_ERR("malloc failed\n");
		error = -ENOMEM;
...
static void ts_charger_detect_register_client(void)
{
	...
	if (error < 0) {
		...
		if(TS_CHARGER_TYPE_SDP == upmu_is_chr_det()){
			charger_state = USB_PIUG_OUT;
		}
		ts_charger_detect_cmd(charger_state);
	}
...
static int get_ts_board_info(void)
{
    ...
    rc = of_property_read_u32_array(g_ts_kit_platform_data.node, "slave_address",slave_addr, ARRAY_SIZE(slave_addr));
    if (rc)
    {
        TS_LOG_ERR("slave_address read failed\n");
        error = -EINVAL;
        goto out;
    }
    for(i=0;i<16;i++)
    {
		TS_LOG_INFO("slave_addr[%d]=0x%x ",i,slave_addr[i]);
		if(slave_addr[i] == 0)
			break;
		else
			g_ts_kit_platform_data.slave_addr[i] = (u8)slave_addr[i];
    }
...
#if defined(HUAWEI_CHARGER_FB)
      ts_charger_detect_register_client();
#endif
...
static void ts_kit_exit(void)
{
	...
	if (NULL != g_ts_kit_platform_data.charger_detect_notify.notifier_call) {
		charger_detect_unregister_client(&g_ts_kit_platform_data. charger_detect_notify);
...
int huawei_ts_chip_register(struct ts_kit_device_data* chipdata)
{
    ...
    {
        if (chipdata->ops->chip_detect && g_ts_kit_platform_data.slave_addr[0] != 0)
        {
			g_ts_kit_platform_data.chip_data = chipdata;
			for(i=0;i<16;i++)
			{
				if(g_ts_kit_platform_data.slave_addr[i] != 0)
				{
					g_ts_kit_platform_data.client->addr = g_ts_kit_platform_data.slave_addr[i];
...
{
	TS_LOG_INFO("MediaTek touch panel driver init\n");
	if (platform_driver_register(&tpd_driver) != 0)
		TS_LOG_INFO("unable to register touch panel driver.\n");
}
...
int huawei_ts_module_init(void)
{
	...
	if (!res)
		TS_LOG_ERR("tpd : touch device init failed res:%d\n", res);
	return 0;
}
/* should never be called */
void  huawei_ts_module_exit(void)
{
	TS_LOG_INFO("MediaTek touch panel driver exit\n");
