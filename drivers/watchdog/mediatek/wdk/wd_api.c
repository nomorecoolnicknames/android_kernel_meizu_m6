...
//changjingyang add for rpmb trigger
...
int check_rtc_status(void)
{
	...
	pr_alert("LCT check_rtc_status flag:%d\n",get_rtc_mark_rpmb());
}
EXPORT_SYMBOL(check_rtc_status);
#endif
//changjingyang add for end
#ifndef CONFIG_MEDIATEK_WATCHDOG
...
void arch_reset(char mode, const char *cmd)
{
	...
	if (cmd && !strcmp(cmd, "charger")) {
		...
		rtc_mark_logging();
		reboot = 1;
	} else if (cmd && !strcmp(cmd, "logging")) {
		rtc_mark_logging();
		reboot = 1;
		...
		/*changjingyang add for RPMB trigger*/
#ifdef CONFIG_LCT_RPMB_TRIGGER
	}else if((cmd && !strcmp(cmd,"rpmb"))||(cmd && !strcmp(cmd,"sdupdate"))){
		reboot = 1;  //changjingyang add for powerkey pass
		rtc_mark_rpmb();
		check_rtc_status();
		pr_alert("LCT get rpmg flag:%d\n",check_rtc_status());
		/*changjingyang add end*/
#endif
	} else {
		reboot = 1;
	}
