...
out:
	if (v->mode == DM_VERITY_MODE_LOGGING)
		return 0;
	if (v->mode == DM_VERITY_MODE_RESTART || v->mode == DM_VERITY_MODE_EIO)
			       ...
			       u8 *want_digest)
{
	...
	if (!aux->hash_verified) {
		...
		{
			#if defined (CONFIG_HUAWEI_DSM)
			verity_dsm(v, DM_VERITY_BLOCK_TYPE_METADATA, hash_block, DSM_DM_VERITY_FEC_INFO_NO);
			#endif
		...
		else
		{
				...
				if (verity_handle_err(v, DM_VERITY_BLOCK_TYPE_METADATA, hash_block))
				{
					r = -EIO;
					goto release_ret_r;
				}
...
static int verity_verify_io(struct dm_verity_io *io)
{
	...
	for (b = 0; b < io->n_blocks; b++) {
		...
		{
			#if defined (CONFIG_HUAWEI_DSM)
			verity_dsm(v, DM_VERITY_BLOCK_TYPE_DATA,io->block + b, DSM_DM_VERITY_FEC_INFO_NO);
			#endif
		...
		else
		{
			...
			if (verity_handle_err(v, DM_VERITY_BLOCK_TYPE_DATA, io->block + b))
			{
				return -EIO;
			}
			else
