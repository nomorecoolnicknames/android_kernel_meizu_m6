...
/********************
 * show GPU freq table// add by zhaofei - 2017-04-07-19-44
 *********************/
static int mt_gpufreq_opp_freq_value_proc_show(struct seq_file *m, void *v)
{
	int i = 0;

	for (i = 0; i < mt_gpufreqs_num; i++) {
		seq_printf(m, "%d ", mt_gpufreqs[i].gpufreq_khz);
	}
...
PROC_FOPS_RO(gpufreq_opp_freq_value);// add by zhaofei - 2017-04-07-19-49
...
static int mt_gpufreq_create_procfs(void)
{
	...
	const struct pentry entries[] = {
		...
		PROC_ENTRY(gpufreq_opp_freq_value),// add by zhaofei - 2017-04-07-19-48
