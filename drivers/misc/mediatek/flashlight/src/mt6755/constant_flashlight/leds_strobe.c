...
/*jijin.wang add & remove for duty*/
...
/*jijin.wang remove not use func*/
#if 0
...
int mt_flash_set_pwm(int pwm_no,int level)
{
	struct pwm_spec_config pwm_setting;
	pwm_setting.pwm_no = pwm_no;
	pwm_setting.mode = PWM_MODE_OLD;
	pwm_setting.pmic_pad = false;
	PK_DBG("led_set_pwm: mode=%d,pwm_no=%d\n",pwm_setting.mode,pwm_setting.pwm_no);
	pwm_setting.clk_src = PWM_CLK_OLD_MODE_BLOCK;//PWM_CLK_OLD_MODE_32K;
	pwm_setting.clk_div = CLK_DIV1;
	pwm_setting.PWM_MODE_OLD_REGS.IDLE_VALUE = 0;
        pwm_setting.PWM_MODE_OLD_REGS.GUARD_VALUE = 0;
        pwm_setting.PWM_MODE_OLD_REGS.GDURATION = 0;
        pwm_setting.PWM_MODE_OLD_REGS.WAVE_NUM = 0;
        pwm_setting.PWM_MODE_OLD_REGS.DATA_WIDTH = 256; /* 256 level */
...
int flashlight_set_onoff(unsigned int onoff)
{
	if(onoff)
	{
        ...
        if(onoff == 1){
            mt_flash_set_pwm(2,60);
        }
        else if(onoff > 1 && onoff <= 256)
        {
            mt_flash_set_pwm(2,onoff);
        }
        else
        {
            flashlight_gpio_set(FLASHLIGHT_PIN_TORCH,STATE_LOW);
            mt_flash_set_pwm(2,0);
            PK_DBG("Flashlight brightness not in scope,input right value(0~256)\n");
        }
		PK_DBG("open Flashlight mode is TORCH for 87 mode!!!\n");
	}
	else
	{
		flashlight_gpio_set(FLASHLIGHT_PIN_TORCH,STATE_LOW);
		mt_flash_set_pwm(2,0);
		PK_DBG("close Flashlight mode is TORCH for 87 mode!!!\n");
	}
	return onoff;
}
...
int FL_Enable(void)
{
	...
	if(g_duty1 == 0){
		flashlight_gpio_set(FLASHLIGHT_PIN_TORCH,STATE_HIGH);
        mt_flash_set_pwm(2,158);
		PK_DBG("set Flashlight mode is TORCH!!!\n");
	}else{
		flashlight_gpio_set(FLASHLIGHT_PIN_FLASH,STATE_HIGH);
        mt_flash_set_pwm(1,128);
		PK_DBG("set Flashlight mode is FLASH!!!\n");
	}
	return 0;
}
