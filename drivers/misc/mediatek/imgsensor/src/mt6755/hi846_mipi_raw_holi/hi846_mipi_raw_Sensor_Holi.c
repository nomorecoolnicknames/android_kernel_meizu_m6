...
#include "hi846_mipi_raw_Sensor_Holi.h"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI846_SENSOR_ID_HOLI,
...
struct HI846_otp_struct_Holi HI846_otp_Holi;


void HI846_OTPSetting_HELI(void)
{
...
static int HI846_otp_read(void)
{
	...
	HI846_otp_Holi.Base_Info_Flag = OTP_read_cmos_sensor(0x0201);
	if (HI846_otp_Holi.Base_Info_Flag == 0x01)	//Base Info Group1 valid
	{
		addr = 0x0202;
	}
	else if (HI846_otp_Holi.Base_Info_Flag == 0x13)	//Base Info Group2 valid
	{
		addr = 0x0213;
	}
	else if (HI846_otp_Holi.Base_Info_Flag == 0x37)	//Base Info Group3 valid
	{
		addr = 0x0224;
	}
	else
	...
	if (addr == 0) {
		HI846_otp_Holi.module_integrator_id = 0;
		HI846_otp_Holi.prodyction_year = 0;
		HI846_otp_Holi.production_month = 0;
		HI846_otp_Holi.production_day = 0;
		HI846_otp_Holi.lens_id = 0;
		HI846_otp_Holi.infocheck = 0;
		HI846_otp_Holi.IR_id = 0;
		return READ_CMOS_FAIL;
	} else {
		HI846_otp_Holi.module_integrator_id = OTP_read_cmos_sensor(addr);
		HI846_otp_Holi.lens_id = OTP_read_cmos_sensor(addr + 1);
		HI846_otp_Holi.prodyction_year = OTP_read_cmos_sensor(addr + 2);
		HI846_otp_Holi.production_month = OTP_read_cmos_sensor(addr + 3);
		HI846_otp_Holi.production_day = OTP_read_cmos_sensor(addr + 4);
		HI846_otp_Holi.IR_id = OTP_read_cmos_sensor(addr + 5);
		HI846_otp_Holi.Gray_wh = OTP_read_cmos_sensor(addr + 6);
		HI846_otp_Holi.Gray_wl = OTP_read_cmos_sensor(addr + 7);
		HI846_otp_Holi.Gray_hh = OTP_read_cmos_sensor(addr + 8);
		HI846_otp_Holi.Gray_hl = OTP_read_cmos_sensor(addr + 9);
		HI846_otp_Holi.infocheck = OTP_read_cmos_sensor(addr + 16);
	}

	checksum = (HI846_otp_Holi.module_integrator_id + HI846_otp_Holi.prodyction_year + HI846_otp_Holi.production_month + HI846_otp_Holi.production_day + HI846_otp_Holi.lens_id +
				HI846_otp_Holi.IR_id + HI846_otp_Holi.Gray_wh+ HI846_otp_Holi.Gray_wl + HI846_otp_Holi.Gray_hh + HI846_otp_Holi.Gray_hl) % 0xFF + 1;
	if (checksum == HI846_otp_Holi.infocheck)
	{
	...
	LOG_INF("HI846 module_integrator_id = 0x%x,  prodyction_year = 0x%x, production_month = 0x%x, production_day = 0x%x,lens_id = 0x%x, infocheck = 0x%x \n",\
		HI846_otp_Holi.module_integrator_id,HI846_otp_Holi.prodyction_year, HI846_otp_Holi.production_month,HI846_otp_Holi.production_day, HI846_otp_Holi.lens_id, HI846_otp_Holi.infocheck);
	HI846_otp_Holi.WB_FLAG = OTP_read_cmos_sensor(0x0C5F);
	if (HI846_otp_Holi.WB_FLAG == 0x01)
	{
		wb_start_addr = 0x0C60;
	}
	else if (HI846_otp_Holi.WB_FLAG == 0x13)
	{
		wb_start_addr = 0x0C7E;
	}
	else if (HI846_otp_Holi.WB_FLAG == 0x37)
	{
	...
	if (wb_start_addr != 0) {
		...
		checksum = 0;
		for (i = 0; i < 30; i++) {
			HI846_otp_Holi.wb_data[i] = read_cmos_sensor_otp(0x708);	//otp data read
			if(i <= 27)
				checksum += HI846_otp_Holi.wb_data[i];
			LOG_INF("HI843 wb_data[%d] = 0x%x\n", i, HI846_otp_Holi.wb_data[i]);
		}
	}
	checksum = checksum % 0xFF + 1;
	LOG_INF("HI843 checksum = 0x%x\n",checksum);
	HI846_otp_Holi.wbcheck = HI846_otp_Holi.wb_data[29];
	/*awb info check*/
	if(HI846_otp_Holi.wbcheck == checksum)
	{
	...
	HI846_otp_Holi.LSC_FLAG = OTP_read_cmos_sensor(0x0235);
	if (HI846_otp_Holi.LSC_FLAG == 0x01)
		lsc_start_addr = 0x0236;
	else if (HI846_otp_Holi.LSC_FLAG == 0x13)
		lsc_start_addr = 0x0599;
	else if (HI846_otp_Holi.LSC_FLAG == 0x37)
	...
	if (lsc_start_addr != 0){
		...
		checksum = 0;
		for (i = 0; i < 867; i++){
			HI846_otp_Holi.lsc_data[i] = read_cmos_sensor_otp(0x708);	//otp data read
			if(i <= 857)
				checksum += HI846_otp_Holi.lsc_data[i];
			//LOG_INF("HI843 lsc_data[%d] = 0x%x\n", i, HI846_otp_Holi.wb_data[i]);
		}
	}
	checksum = checksum%0xFF + 1;
	HI846_otp_Holi.lsc_check = HI846_otp_Holi.lsc_data[866];
	/*awb info check*/
	if(HI846_otp_Holi.lsc_check == checksum)
	{
...
static int HI846_otp_apply(void)
{
	...
	RG_ratio_unit = (((((HI846_otp_Holi.wb_data[12] << 8)|HI846_otp_Holi.wb_data[13])) * 1024)/(((HI846_otp_Holi.wb_data[16] << 8)|HI846_otp_Holi.wb_data[17] )));
	BG_ratio_unit = (((((HI846_otp_Holi.wb_data[14] << 8)|HI846_otp_Holi.wb_data[15])) * 1024)/(((HI846_otp_Holi.wb_data[18] << 8)|HI846_otp_Holi.wb_data[19] )));
	RG_ratio_golden = 0x1c6;
	BG_ratio_golden = 0x281;
...
int Gudge_ModuleIsTrue_Holi(void)
{
	int addr = 0;
	...
	if(OTP_read_cmos_sensor(addr) == 0x42){
		#ifdef CONFIG_HUAWEI_HW_I2C_DCT
			...
			do {
				*sensor_id =return_sensor_id() + 1;
				...
				if (*sensor_id == imgsensor_info.sensor_id){
					...
					#endif
					app_info_set("camera_slave","HI846B-HELI");
					return ERROR_NONE;
				}
...
#endif
	//mtk_otp_check_set("slave_camera",0);
	spin_lock(&imgsensor_drv_lock);
...
UINT32 HI846_MIPI_RAW_SensorInit_Holi(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	HI846_MIPI_RAW_SensorInit_Holi*/
