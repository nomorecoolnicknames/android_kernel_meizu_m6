...
extern int bluetooth_dsm_report_num(int dsm_err_no, char *err_msg, int err_code);
#endif
...
#ifdef CONFIG_HUAWEI_DSM
	if(i_ret < 0) {
		bluetooth_dsm_report_num(DSM_BT_PROBE_ERROR,"start to do bluetooth driver init fail!", i_ret);
	}
#endif
#else
