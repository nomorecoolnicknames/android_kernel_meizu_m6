...
#ifdef CONFIG_HUAWEI_DSM
		wifi_dsm_report_num(DSM_WIFI_DRIVER_LOAD_ERR,"DSM_WIFI_FAIL_LOADFIRMWARE_ERR:Load FW image failed", IS_ERR(filp));
#endif
		goto error_open;
	}
#if 0
...
WLAN_STATUS kalFirmwareLoad(IN P_GLUE_INFO_T prGlueInfo, OUT PVOID prBuf, IN UINT_32 u4Offset, OUT PUINT_32 pu4Size)
{
	...
	if ((filp == NULL) || IS_ERR(filp) || (filp->f_op == NULL) || (filp->f_op->read == NULL)) {
		...
		*pu4Size = filp->f_op->read(filp, prBuf, *pu4Size, &filp->f_pos);
	       */
               DBGLOG(INIT, INFO, "kalFirmwareLoad read start!\n");
               if(vfs_llseek(filp, u4Offset, SEEK_SET) != u4Offset)
              {
                  DBGLOG(INIT, INFO, "failed to seek in  kalFirmwareLoad! \n");
                  goto error_read;
              }
               ...
               while (pos < *pu4Size) {
                   offset1 = pos;
                   pos = vfs_read(filp, (void __user *)(prBuf) + pos, *pu4Size - pos, &offset1);
		   if (bytes < 0) {
                       //DBGLOG(INIT,  INFO, "failed to read in kalFirmwareLoad!\n", );
		       goto error_read;
		  }
		  if (bytes == 0)
			break;
