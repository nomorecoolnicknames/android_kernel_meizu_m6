			      ...
			      IN struct iw_request_info *info, IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	if ((prGlueInfo->prP2PInfo->u4ConnReqNameLength >= sizeof(prDeviceReq->name)) || (prGlueInfo->prP2PInfo->u4ConnReqNameLength <= 0) || !prGlueInfo->prP2PInfo->aucConnReqDevName) {
	DBGLOG(P2P, ERROR, "%s Setting wrong parameters to wifi p2p driver!!!\n", __func__);
	return -EFAULT;
	}
