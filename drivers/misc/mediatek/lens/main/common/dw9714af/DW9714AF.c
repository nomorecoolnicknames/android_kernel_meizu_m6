...
static void initdrv(void)
{
    ...
    char puSendCmd2[2] = { 0x02, 0x00};//0x48
    char puSendCmd3[2] = { 0x06, 0x84};
    char puSendCmd4[2] = { 0x07, 0x01};
    char puSendCmd5[2] = { 0x08, 0x32};
...
#ifdef CONFIG_HUAWEI_DSM
	curPos = g_u4CurrPosition;
	tarPos = g_u4TargetPosition;
#endif
...
#ifdef CONFIG_HUAWEI_DSM
		camera_report_lens_dsm_err(AF_DRVNAME, curPos, tarPos, DSM_CAMERA_ACTUATOR_MOVE_FAIL, -1);
#endif
...
int DW9714AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (*g_pAF_Opened == 2) {
		LOG_INF("Wait\n");
        //LOG_INF("Release g_u4CurrPosition = %ld\n",g_u4CurrPosition);
        if(g_u4CurrPosition > 250)
        {
            s4AF_WriteReg(250);
            mdelay(20);
            g_u4CurrPosition = 250;
        }
        while(g_u4CurrPosition > 30)
        {
            g_u4CurrPosition = g_u4CurrPosition - 30;
            //LOG_INF("g_u4CurrPosition = %ld\n",g_u4CurrPosition);
