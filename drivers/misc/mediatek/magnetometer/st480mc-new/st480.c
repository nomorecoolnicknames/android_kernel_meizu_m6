...
struct mag_hw mag_cust2;
static struct mag_hw *hw = &mag_cust2;
...
static int ms_report_dsm_err(int type)
{
    ...
    {
            /* buffer is busy */
            MSE_ERR("%s: buffer is busy!", __func__);
            return -EBUSY;
    }
    switch(type)
    {
        ...
        case DSM_SHB_ERR_MAG_DATA_ABNORAML:
            ...
            break;
        default:
            MSE_ERR("%s:unsupported dsm report type.\n",__func__);
            break;
        }
    total_size += size;
...
**/
	err = app_info_set("M-Sensor", "ST480");
	if(err < 0){
	    MSE_LOG("st480 failed to add app_info\n");
	}
...
static void st480_work_func(void)
{
	...
	MSE_LOG("st480 raw data: x = %d, y = %d, z = %d\n",mag.mag_x,mag.mag_y,mag.mag_z);
	#ifdef CONFIG_HUAWEI_DSM
	if(mag.mag_x == 0 && mag.mag_y == 0 && mag.mag_z == 0)
	{
		MSE_ERR("%s:Invalid data: x,y,z all is 0.\n", __func__);
...
#ifdef CONFIG_HUAWEI_DSM
static ssize_t st480_show_test_dsm(struct device_driver *ddri, char *buf)
{
...
*/
static ssize_t st480_store_test_dsm(struct device_driver *ddri, const char *buf, size_t tCount)
{
    int mode;
    int ret = 0;
...
#ifdef CONFIG_HUAWEI_DSM
static DRIVER_ATTR(dsm_excep, 0660, st480_show_test_dsm, st480_store_test_dsm);
#endif
/*----------------------------------------------------------------------------*/
