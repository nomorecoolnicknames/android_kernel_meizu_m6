...
static void mdee_dumper_info_prepare_v1(struct md_ee *mdee)
{
	...
	case MD_EX_TYPE_ASSERT:
        ...
        snprintf(debug_info->assert.file_name, sizeof(debug_info->assert.file_name),"%s",
	...
	case DSP_EX_TYPE_ASSERT:
        ...
        snprintf(debug_info->dsp_assert.file_name, sizeof(debug_info->dsp_assert.file_name),"%s",
            ex_info->content.assert.filename);
		debug_info->dsp_assert.line_num = ex_info->content.assert.linenumber;
		snprintf(debug_info->dsp_assert.execution_unit, sizeof(debug_info->dsp_assert.execution_unit),"%s",
	...
	case DSP_EX_TYPE_EXCEPTION:
		...
		snprintf(debug_info->dsp_exception.execution_unit, sizeof(debug_info->dsp_exception.execution_unit),"%s",
			 ex_info->envinfo.execution_unit);
	...
	case DSP_EX_FATAL_ERROR:
		...
		snprintf(debug_info->dsp_fatal_err.execution_unit, sizeof(debug_info->dsp_fatal_err.execution_unit),"%s",
			 ex_info->envinfo.execution_unit);
