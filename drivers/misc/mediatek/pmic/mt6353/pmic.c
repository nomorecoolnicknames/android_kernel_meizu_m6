...
int pmic_chk_intf(void)
{
	...
	if (mutex_is_locked(&pmic_access_mutex)) {
		if (pmic_access_mutex.owner) {
			...
			pr_debug("pmic_access_mutex owner have been clear\n");
	} else
		return 0;
				 ...
				 size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		if (size > 5) {
			...
			if(ret!=0){
				pr_err("[store_pmic_access]pmic_config_interface ERROR\n");
				return ret;
				}
		} else {
			ret = pmic_read_interface(reg_address, &g_reg_value, 0xFFFF, 0x0);
			if(ret!=0){
				pr_err("[store_pmic_access]pmic_read_interface ERROR\n");
				return ret;
				}
