...
static struct notifier_block step_counter_fb_notifier;
...
static void step_c_work_func(struct work_struct *work)
{
	...
	step_c_data_report(cxt->idev, cxt->drv_data.counter, cxt->drv_data.status, nt);
step_c_loop:
...
int step_notify(STEP_NOTIFY_TYPE type)
{
	...
	if (type == TYPE_STEP_DETECTOR) {
		...
		input_report_rel(cxt->idev, EVENT_TYPE_STEP_TIMESTAMP_HI, nt >> 32);
		input_report_rel(cxt->idev, EVENT_TYPE_STEP_TIMESTAMP_LO, nt & 0xFFFFFFFFLL);
		input_sync(cxt->idev);
...
#endif
static int step_counter_suspend(void)
{
	struct step_c_context *cxt = NULL;
	cxt = step_c_context_obj;
	del_timer_sync(&cxt->timer);
	STEP_C_ERR(" step_counter_suspend\n");
	return 0;
}

static int step_counter_resume(void)
{
	struct step_c_context *cxt = NULL;
	cxt=step_c_context_obj;
	mod_timer(&cxt->timer,jiffies + atomic_read(&cxt->delay)/(1000/HZ));
	STEP_C_ERR(" step_counter_resume\n");
	return 0;
}
...
int step_c_driver_add(struct step_c_init_info *obj)
{
	...
	if(i>=MAX_CHOOSE_STEP_C_NUM){
	         STEP_C_ERR("step_c_driver_add exceed range not eorroor\n");
	        i=MAX_CHOOSE_STEP_C_NUM-1;
	}
	if (NULL == step_counter_init_list[i]) {
...
static int step_c_input_init(struct step_c_context *cxt)
{
	...
	input_set_capability(dev, EV_REL, EVENT_TYPE_STEP_TIMESTAMP_HI);
	input_set_capability(dev, EV_REL, EVENT_TYPE_STEP_TIMESTAMP_LO);
...
int step_c_data_report(struct input_dev *dev, uint32_t value, int status, int64_t nt)
/*dingleilei*/
{
	...
	/*dingleilei*/
	//input_report_rel(dev, EVENT_TYPE_STEP_UPDATE, 1);
	input_report_rel(dev, EVENT_TYPE_STEP_TIMESTAMP_HI, nt >> 32);
	input_report_rel(dev, EVENT_TYPE_STEP_TIMESTAMP_LO, nt & 0xFFFFFFFFLL);
	/*dingleilei*/
	input_sync(dev);
	return 0;
}
static int step_counter_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
    struct fb_event *evdata = NULL;
    int blank;
    int err = 0;

    STEP_C_LOG("step_counter_fb_notifier");
    ...
    STEP_C_LOG("fb_notify(blank=%d)", blank);
    switch (blank) {
    case FB_BLANK_UNBLANK:
        STEP_C_LOG("LCD ON Notify");
        err = step_counter_resume();
        if (!err) {
            STEP_C_LOG("start step_counter_resume failed");
            return err;
            }
        break;
    case FB_BLANK_POWERDOWN:
        STEP_C_LOG("LCD OFF Notify");
        err = step_counter_suspend();
        if (!err)
            STEP_C_LOG("cancel touch_resume_workqueue err = %d", err);
        break;
    default:
        break;
    }
...
static int step_c_probe(void)
{
	...
	step_counter_fb_notifier.notifier_call = step_counter_fb_notifier_callback;
	if (fb_register_client(&step_counter_fb_notifier))
		STEP_C_LOG("register fb_notifier fail!");
