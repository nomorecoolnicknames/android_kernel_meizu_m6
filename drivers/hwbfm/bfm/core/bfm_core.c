...
/**
    @function: int bfm_copy_fail_info_raw_part(char *raw_part_name, unsigned long long offset, char *buf, unsigned int log_len)
    @brief: copy bootfail info or recovery info to raw partition
...
/*
Total : 768432B
0 ~ 512           for error inject
512 ~ 1024      for kernel error log, now not use
1024 ~ 4096     for reserved
4096 ~ 4096+4*1024*10     for bootfail_info.txt
4096+4*1024*10 ~ 4096+4*1024*10+4*1024*10 for recoveryinfo.txt
768432-512 ~ 768432 for save log num count flag
*/
                                   ...
                                   unsigned long long offset, char *buf, unsigned int log_len)
{
    ...
    ret = bfmr_save_log_to_raw_part(raw_part_name, (unsigned long long)(offset + bfm_wflag*LOG_SIZE), pdata, LOG_SIZE);
    if (ret)
    {
      ret = -1;
    ...
    ret = bfmr_save_log_to_raw_part(raw_part_name, (unsigned long long)(offset + bfm_wflag*LOG_SIZE), (char*)buf_log_data, sizeof(struct hwbfm_log));
    if (ret)
    {
      ret = -1;
        ...
        case DST_FILE:
            {
                ...
                bfm_copy_bootfail_info_to_raw_part(BFR_PARTITION, (unsigned long long)BOOTFAIL_OFFSET, pdata, strlen(pdata));
                break;
            }
        case DST_RAW_PART:
            {
...
static int bfm_process_bottom_layer_boot_fail(void *param)
{
    ...
    if( i >= count )
    {
        BFMR_PRINT_ERR("bfmr length failed!\n");
        goto __out;
    }
