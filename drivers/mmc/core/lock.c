...
extern int mmc_devfreq_create_freq_table(struct mmc_host *host);
#endif
...
static int mmc_lock_sd_init_card(struct mmc_card *card, bool binit)
{
	...
	mmc_devfreq_create_freq_table(card->host);
	printk("[SDLOCK] %s clk_scaling_highest = %lu clk_scaling_lowest = %lu\n",
		__func__, card->clk_scaling_highest, card->clk_scaling_lowest);
#endif
		...
		const char *data, size_t len)
{
	...
	if (mmc_card_locked(card) && !strncmp(command, "erase", 5)) {
		...
		printk("[SDLOCK]  %s: assign sd card \n", __func__);
		//ret = sd_reinit(card, dev);
		ret = sd_lock_do_assign(card, dev, key_buffer, key_len);
		printk("[SDLOCK]  %s: assign sd card result: %d \n", __func__,ret);
		if (MMC_SUCCESS == ret) {
		...
		printk("[SDLOCK] %s: unlock sd card \n", __func__);
