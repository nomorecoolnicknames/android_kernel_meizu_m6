...
static int himax_tpd_rst_gpio_number=2;
static int himax_tpd_int_gpio_number=1;
#endif
...
#endif
#endif
#if 0 //MTK_KERNEL_318 yufangfang
int i2c_himax_read(struct i2c_client *client, uint8_t command, uint8_t *data, uint8_t length, uint8_t toRetry)
{
	int ret;
    ...
    ret = i2c_himax_read_1(client,buf,1,data,length);
	return ret;	
...
int i2c_himax_read_1(struct i2c_client *client, char *writebuf,int writelen, char *readbuf, int readlen)
{
  int ret=0;
	...
	printk("yufangfang himax i2c read1\n");
	...
	if((NULL!=client) && (writelen>0) && (writelen<=128))
	{
		...
		//printk("himax yufangfang%s i2c write len=%x,buffaddr=%x\n", __func__,ret,g_dma_buff_pa);	
		printk("yufangfang himax write\n");
		client->addr = (client->addr & I2C_MASK_FLAG) &(~ I2C_DMA_FLAG);
	}
	...
	if((NULL!=client) && (readlen>0) && (readlen<=128))
	{
		...
		printk("yufangfang i2c himax read len\n");
		memcpy(readbuf, g_dma_buff_va, readlen);

		client->addr = (client->addr & I2C_MASK_FLAG) &(~ I2C_DMA_FLAG);
	}
	printk("yufangfang ret = %d\n",ret);
...
int himax_gpio_power_config(struct i2c_client *client )
{
	...
	TPD_DMESG("mtk_tpd: yufangfang himax reset over himax_tpd_rst_gpio_number = %d\n",himax_tpd_rst_gpio_number);
