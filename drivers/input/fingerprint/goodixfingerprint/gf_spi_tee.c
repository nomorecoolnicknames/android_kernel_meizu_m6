...
//oujiacheng@wind-mobi.com 20170316 begin
//#define GF_IC_NAME "GF3258_YJ "
char *fingerPrinter_name;
//oujiacheng@wind-mobi.com 20170316 end
/* debug log setting */
...
static const struct of_device_id gf_of_match[] = {
	...
	{ .compatible = "goodix,fingerprint1", },
	{},
};
...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	if (IS_ERR(gf_dev->pins_ldo_low)) {
		...
		gf_debug(ERR_LOG, "%s can't find fingerprint pinctrl pins_ldo_low\n", __func__);
		return ret;
	}
	...
	if (IS_ERR(gf_dev->pins_ldo_high)) {
		...
		gf_debug(ERR_LOG, "%s can't find fingerprint pinctrl pins_ldo_high\n", __func__);
		return ret;
	}
...
//		hwPowerOn(MT6331_POWER_LDO_VIBR, VOL_2800, "fingerprint");
		enable = 0;
#ifdef CONFIG_OF
		err = pinctrl_select_state(gf_dev->pinctrl_gpios,gf_dev->pins_ldo_high);
		mdelay(10);
...
#ifdef CONFIG_OF
		err = pinctrl_select_state(gf_dev->pinctrl_gpios,gf_dev->pins_ldo_low);
		mdelay(10);
#endif
		enable = 1;
	}
...
/*wangshuangjun@wind-mobi.com  start 20170214*/
...
/*wangshuangjun@wind-mobi.com  start 20170214*/
	...
	case GF_IOC_ENABLE_IRQ:
		gf_debug(INFO_LOG, "%s: GF_IOC_ENABLE_IRQ ======\n", __func__);
                printk("----wind_enirq-----io_enirq\n");
		gf_enable_irq(gf_dev);
		break;

	case GF_IOC_DISABLE_IRQ:
		gf_debug(INFO_LOG, "%s: GF_IOC_DISABLE_IRQ ======\n", __func__);
        printk("----wind_disirq-----io_disirq\n");
		gf_disable_irq(gf_dev);
		break;
...
static int gf_probe(struct spi_device *spi)
{
	...
	gf_spi_clk_enable(gf_dev, 0);
    //oujiacheng@wind-mobi.com 20160316 begin
	fingerPrinter_name = "goodix_fp_yeji";
 //  strcat(fingerPrinter_name,GF_DEV_NAME);
    //oujiacheng@wind-mobi.com 20160316 end
	FUNC_EXIT();
	return 0;
...
static int __init gf_init(void)
{
	...
	err = app_info_set("Fingerprint-Sensor", "GOODIX GF3258");
	if (err < 0)/*failed to add app_info*/
	{
		printk("%s %d:failed to add fingerprint app_info\n", __func__, __LINE__);
	}
	else
	{
		printk("%s:Fingerprint app_info GOODIX GF3258\n", __func__);
	}
	FUNC_ENTRY();
